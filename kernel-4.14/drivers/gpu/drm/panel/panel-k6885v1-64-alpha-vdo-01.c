...
#ifdef VENDOR_EDIT
/* liwei.a@PSW.MM.Display.LCD.Stability, 2019/10/26, add for hbm*/
static unsigned int g_hbm_mode = 2;
static unsigned int backup_bl = 0;
#endif
...
static const struct drm_display_mode default_mode = {
	...
	.hsync_end = 1080 + 100 + 40,
	.htotal = 1080 + 100 + 40 + 50,
	.vdisplay = 2340,
	.vsync_start = 2340 + 5,
	.vsync_end = 2340 + 5 + 2,
	.vtotal = 2340 + 5 + 2 + 5,
		...
		void *handle, unsigned int level)
{
    ...
    pr_err("in %s, 0x51 write 0x%x, 0x%x\n", __func__, bl_tb0[1], bl_tb0[2]);
...
{
	char hbm_on0[] = {0x53, 0xE0};
	char hbm_on1[] = {0x51, 0x07, 0xFF};
	char hbm_off0[] = {0x53, 0x20};
	char hbm_off1[] = {0x51, 0x00, 0xFF};

	hbm_off1[1] = backup_bl >> 8;
	hbm_off1[2] = backup_bl & 0xFF;

	if (!cb)
		return -1;

	pr_err("g_hbm_mode= %u, hbm_mode=%u\n", g_hbm_mode, hbm_mode);
	if(g_hbm_mode != hbm_mode) {
		if(hbm_mode == 1) {
			cb(dsi, handle, hbm_on0, ARRAY_SIZE(hbm_on0));
            cb(dsi, handle, hbm_on1, ARRAY_SIZE(hbm_on1));
		} else if (hbm_mode == 0) {
			cb(dsi, handle, hbm_off0, ARRAY_SIZE(hbm_off0));
