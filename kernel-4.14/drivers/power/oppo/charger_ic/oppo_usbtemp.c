...
int oppo_usbtemp_dischg_action(struct oppo_chg_chip *chip)
{
	...
	pr_err("[oppo_usbtemp_dischg_action]:run_action");
	if(chip->chg_ops->set_typec_sinkonly != NULL){
		chip->chg_ops->set_typec_sinkonly();
	} else {
		pr_err("[oppo_usbtemp_dischg_action]: set_typec_sinkonly is null");
	}
...
#else
	pr_err("[oppo_usbtemp_dischg_action]: set vbus down");
...
int oppo_usbtemp_monitor_common(void *data)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
#endif
	pr_err("[oppo_usbtemp_monitor_main]:run first!");
	...
	while (!kthread_should_stop()) {
		if(chip->chg_ops->oppo_usbtemp_monitor_condition != NULL){
			...
			pr_err("[oppo_usbtemp_monitor_main]:condition pointer is NULL");
			return 0;
		}
		...
		if(chip->chg_ops->get_usbtemp_volt != NULL){
			...
			pr_err("[oppo_usbtemp_monitor_main]:get_usbtemp_volt is NULL");
			return 0;
		}
		get_usb_temp(chip);
