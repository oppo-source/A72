...
static int pm8008_regulator_probe(struct platform_device *pdev)
{
    ...
    pr_err("pm8008_regulator_probe end\n");
...
static int pm8008_chip_probe(struct platform_device *pdev)
{
	...
	if (!chip->regmap) {
		pr_err("PM8008 parent regmap is missing\n");
		return -EINVAL;
	}
	chip->dev = &pdev->dev;
