...
static void read_eeprom_SN(void)
{
	...
	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(OV48B_STEREO_START_ADDR_WIDE+idx)>>8,
		(char)((OV48B_STEREO_START_ADDR_WIDE + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&gOtpCheckdata[0][idx],1, 0xA0);
		LOG_INF("OV48B_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(OV48B_STEREO_START_ADDR_TELE+idx)>>8,
		(char)((OV48B_STEREO_START_ADDR_TELE + idx) & 0xFF) };
