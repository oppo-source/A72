...
#define DEVICE_MANUFACUTRE_HLT        "Holitech"
#define DEVICE_MANUFACUTRE_TSP         "TSP"
...
#define IMGSENSOR_MODULE_ID_TSP         0x1B
...
void register_imgsensor_deviceinfo(char *name, char *version, u8 module_id)
{
    ...
    switch (module_id)
    {
        ...
        case IMGSENSOR_MODULE_ID_HLT:  /* HLT */
            ...
            break;
        case IMGSENSOR_MODULE_ID_TSP:  /* TSP*/
            manufacture = DEVICE_MANUFACUTRE_TSP;
            break;
        default:
	...
	PK_INFO("%s secure state %d", __func__,
		...
		if (ret != ERROR_NONE) {
			...
			if (is_project(OPPO_19131)||is_project(OPPO_19132)||is_project(OPPO_19133)||is_project(OPPO_19420)
				...
				||is_project(OPPO_20003)||is_project(OPPO_20200)||is_project(OPPO_20041)||is_project(OPPO_20042)){
...
int imgsensor_set_driver(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (is_project(OPPO_19040)) {
		...
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
			imgsensor_custom_config_20041[psensor_inst->sensor_idx].i2c_dev);
	} else {
		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
