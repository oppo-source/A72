...
#define DEVICE_VERSION_IMX319    "imx319"
...
static kal_uint8 gIMX319_SN[CAMERA_MODULE_SN_LENGTH];
static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gIMX319_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		...
		LOG_INF("gIMX319_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gIMX319_SN[idx]);
	}

	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(IMX319_STEREO_START_ADDR+idx)>>8,
			(char)((IMX319_STEREO_START_ADDR + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&gOtpCheckdata[1][idx],1, 0xA8);
		LOG_INF("gOtpCheckdata[1][%d]: 0x%x\n", idx, get_byte[idx]);
	}
}

/*Henry.Chang@camera.driver 20181129, add for sensor Module SET*/
#define   WRITE_DATA_MAX_LENGTH                (16)
#define   IMX319_STEREP_DATA_ADDR_START         (0x1E80)
#define   IMX319_STEREP_DATA_LENGTH             (1561)
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("imx319 SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == IMX319_SENSOR_ID)
			...
			write_eeprom_protect(1);
			msleep(6);
			pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR));
			pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+39));
			pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+40));
			pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+904));
			pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+905));
			pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+906));
			pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+907));
			pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(IMX319_STEREO_START_ADDR+908));
			pr_debug("imx319write_Module_data Write end\n");
		} else if ((pStereodata->uSensorId == IMX319_SENSOR_ID)
			&& (data_base == IMX319_AESYNC_START_ADDR)
			...
			pr_debug("readback TELE aesync: %x %x %x %x %x %x %x %x\n"
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+1)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+2)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+3)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+4)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+5)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+6)
				, read_cmos_eeprom_8(IMX319_AESYNC_START_ADDR+7));
		} else {
			pr_err("Invalid Sensor id:0x%x write_gm1 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("imx319 write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[3] = {
	...
	{0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x0900, 0x06c0, 0x00, 0x00, 0x0000, 0x0000,
	 0x00, 0x36, 0x03ca, 0x0001, 0x00, 0x00, 0x0000, 0x0000},
			...
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
                    ...
                    if (deviceInfo_register_value == 0x00) {
                        register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_IMX319,
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_MODULE_SN:
        ...
        if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
            *(feature_data_32 + 1) = (gIMX319_SN[4*(*feature_data_32) + 3] << 24)
                        | (gIMX319_SN[4*(*feature_data_32) + 2] << 16)
                        | (gIMX319_SN[4*(*feature_data_32) + 1] << 8)
                        | (gIMX319_SN[4*(*feature_data_32)] & 0xFF);
    break;
    /*Henry.Chang@camera.driver 20181129, add for sensor Module SET*/
