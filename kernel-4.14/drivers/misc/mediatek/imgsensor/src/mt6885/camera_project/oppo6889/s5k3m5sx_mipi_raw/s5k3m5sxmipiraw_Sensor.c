...
#ifdef VENDOR_EDIT
#define DEVICE_VERSION_S5K3M5SX     "s5k3m5sx"
...
//Feiping.li, add for otp (qcom & mtk) comptile
static int gMTKOTP = 1;
#endif
...
static void read_eeprom_CamInfo(void)
{
	...
	//--- yearH(20)---yearL(19)----------------month-----------------day---------
	if ((get_byte[5]+get_byte[4]) >= 0x27 && (get_byte[3] >= 0x0c) && (get_byte[2] > 0x01)) {  //module after 2019.12.1 support PDAF
		gMTKOTP = 1;
	} else {
		gMTKOTP = 0;
	}
...
static void read_eeprom_SN(void)
{
	...
	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(S5K3M5SX_STEREO_START_ADDR+idx)>>8, (char)((S5K3M5SX_STEREO_START_ADDR + idx) & 0xFF)};
		iReadRegI2C(pusendcmd , 2, (u8*)&gOtpCheckdata[3][idx],1, 0xA0);
		if (gOtpCheckdata[3][idx] != gOtpCheckdata[6][idx]) {
			LOG_INF("[%d]: 0x%x!=0x%x\n", idx, gOtpCheckdata[3][idx], gOtpCheckdata[6][idx]);
			return;
		}
		LOG_INF("s5k3m5SX[%d]: 0x%x\n", idx, gOtpCheckdata[3][idx]);
	}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == S5K3M5SX_SENSOR_ID)
			...
			pr_debug("s5k3m5 Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[904],pData[905], pData[906], pData[907], pData[908]);
...
static void s5k3m5sx_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void s5k3m5sx_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				*sensor_id = S5K3M5SX_SENSOR_ID;
				...
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_S5K3M5SX, imgsensor_info.module_id);
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		s5k3m5sx_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
			, feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
		 	(*feature_para_len));
		s5k3m5sx_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
			, feature_data_16);
		break;
