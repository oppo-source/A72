...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == GC8054_SENSOR_ID)
			...
			msleep(6);
			if (ret != IMGSENSOR_RETURN_SUCCESS) {
				LOG_INF("write TELE_aesync_eeprom error\n");
				return IMGSENSOR_RETURN_ERROR;
			}
			...
			LOG_INF("Invalid Sensor id:0x%x write_gm1 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		LOG_INF("gc8054write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO:
		...
		LOG_INF("zhuangzhuang = %d\n", *(MUINT32 *)(uintptr_t)(*(feature_data + 1)));
        break;
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3800000;
		break;
