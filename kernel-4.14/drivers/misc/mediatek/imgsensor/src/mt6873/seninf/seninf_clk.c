...
enum SENINF_RETURN seninf_clk_init(struct SENINF_CLK *pclk)
{
	...
	printk("[%s][seninf_lock_wakelock]",__func__);
	...
	printk("[%s][seninf_lock_wakelock] CONFIG_PM_WAKELOCKS",__func__);

	wakeup_source_init(&pclk->seninf_wake_lock, "seninf_lock_wakelock");
#else
	printk("[%s][seninf_lock_wakelock] WAKE_LOCK_SUSPEND",__func__);
	...
	printk("[%s][seninf_lock_wakelock] wakelock_cnt:%d",__func__,pclk->wakelock_cnt.counter);
...
void seninf_clk_open(struct SENINF_CLK *pclk)
{
	...
	PK_DBG("open\n");
	printk("[%s][seninf_lock_wakelock] wakelock_cnt 1:%d",__func__,pclk->wakelock_cnt.counter);
	...
	printk("[%s][seninf_lock_wakelock] wakelock_cnt 2:%d",__func__,pclk->wakelock_cnt.counter);

	for (i = SENINF_CLK_IDX_SYS_MIN_NUM;
		i < SENINF_CLK_IDX_SYS_MAX_NUM;
		i++) {
		if (clk_prepare_enable(pclk->mclk_sel[i])) {
			printk("[CAMERA SENSOR] ***open1***sys idx= %d\n", i);
			printk("[CAMERA SENSOR] failed sys idx= %d\n", i);
		}
		else
			atomic_inc(&pclk->enable_cnt[i]);
		printk("[CAMERA SENSOR]*****open else****sys idx= %d\n", i);
	}
}
...
void seninf_clk_release(struct SENINF_CLK *pclk)
{
	...
	PK_DBG("release\n");
	printk("[%s][seninf_lock_wakelock] wakelock_cnt 3:%d",__func__,pclk->wakelock_cnt.counter);
	...
	do {
		...
		for (; atomic_read(&pclk->enable_cnt[i]) > 0;) {
			...
			printk("[CAMERA SENSOR] ******release****sys idx= %d\n", i);
		}
	} while (i);
	...
	printk("[%s][seninf_lock_wakelock] wakelock_cnt 4:%d",__func__,pclk->wakelock_cnt.counter);
}
