...
struct lux_aod_event_t {
    int16_t state;
    uint16_t report_count;
};

struct free_fall_event_t{
	uint32_t free_fall_time;
	uint32_t angle;
	uint16_t report_count;
};

struct pickup_motion_event_t{
	uint32_t value;
	uint16_t report_count;
};

struct action_detect_event_t{
	uint32_t value;
	uint16_t report_count;
};
...
struct data_unit_t {
	...
	union {
		...
		//ifdef VENDOR_EDIT
		//tangjh@PSW.BSP.sensor,2019/11/09, add sensor
		struct lux_aod_event_t lux_aod_event;
 		struct free_fall_event_t free_fall_event;
		struct pickup_motion_event_t pickup_motion_event;
 		struct action_detect_event_t action_detect_event;
		struct ffd_event_t ffd_event;
		//endif
		int32_t data[8];
	};
} __packed;
...
struct SCP_SENSOR_HUB_SYNC_UTC{
	enum CUST_ACTION    action;
	union{
...
//#ifdef VENDOR_EDIT
//zhouxu@PSW.BSP.sensor,2019/11/21, add utc print
		struct SCP_SENSOR_HUB_RW_REG rwReg;
		struct SCP_SENSOR_HUB_SYNC_UTC syncUTC;
//#endif//VENDOR_EDIT
