...
//#ifdef VENDOR_EDIT
/* laq@PSW.BSP.sensor,2020/05/21, add for tp vendor adapt */
...
static uint8_t g_tp_dev_vendor;
//#endif
...
static int sensor_list_proc_show(struct seq_file *m, void *v)
{
    ...
    for (handle = accel_handle; handle < maxhandle; handle++) {
        ...
        if (type < 0)
            continue;
        p = &sensor_state[type];
        seq_printf(m, "sensortype:%d vendor:%s chipname:%s\n",
...
//#ifdef VENDOR_EDIT
/* laq@PSW.BSP.sensor,2020/05/21, add for tp vendor adapt */
		data[1] = g_tp_dev_vendor;
		pr_notice("%s als_factor = %d, tp_vendor = %d\n",
...
//#ifdef VENDOR_EDIT
/* laq@PSW.BSP.sensor,2020/05/21, add for tp vendor adapt */
	if (strstr(boot_command_line, "nt36672c")) {
		pr_info("touch ic = nt36672c_jdi \n");
		g_tp_dev_vendor = TP_JDI;
	} else if (strstr(boot_command_line, "hx83112f")) {
		pr_info("touch ic = hx83112f_tianma \n");
