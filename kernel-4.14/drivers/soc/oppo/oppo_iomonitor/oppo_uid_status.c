...
DECLARE_HASHTABLE(oppo_hash_table, UID_HASH_BITS);
static DEFINE_RT_MUTEX(oppo_uid_lock);
...
static int task_limit_count = 0;
static int uid_limit_count = 0;
struct io_stats {
	u64 read_bytes;
	u64 write_bytes;
	u64 rchar;
...
static struct uid_entry *find_uid_entry(uid_t uid)
{
	...
	hash_for_each_possible(oppo_hash_table, uid_entry, hash, uid) {
...
static struct uid_entry *find_or_register_uid(uid_t uid)
{
	...
	hash_add(oppo_hash_table, &uid_entry->hash, uid);
	uid_limit_count++;
		...
		struct task_struct *task, int state)
{
	...
	if((task_all_bytes(task) > TGID_TASK_LIMTI_DATA)){
		...
		if(tgid_task)
			__get_task_comm(task_entry->tgid_comm, TASK_COMM_LEN, tgid_task);
	}
	task_entry->pid = pid;
		...
		struct task_struct *task, int slot, int state)
{
	...
	struct task_entry *task_entry = find_or_register_task(uid_entry, task, state);
	if(!task_entry)
		return;
...
static void show_io_uid_tasks(struct seq_file *m, struct uid_entry *uid_entry)
{
	...
	hash_for_each(uid_entry->task_entries, bkt_task, task_entry, hash) {
		...
		task_all_bytes = task_entry->io[UID_STATE_DELTA].read_bytes + task_entry->io[UID_STATE_DELTA].write_bytes;
		if( task_all_bytes >= TASK_SHOW_DAILY_LIMIT)
			seq_printf(m, "task:%s %lu %llu %llu %llu %llu %llu %d %s\n",
					task_entry->comm,
...
static void remove_no_task_uid_entry(void)
{
	...
	hash_for_each_safe(oppo_hash_table, bkt,
			tmp_task, uid_entry, hash) {
...
static int uid_io_show(struct seq_file *m, void *v)
{
	...
	rt_mutex_lock(&oppo_uid_lock);
	hash_for_each(oppo_hash_table, bkt, uid_entry, hash) {
		...
		all_bytes = uid_entry->io[UID_STATE_DELTA].read_bytes + uid_entry->io[UID_STATE_DELTA].write_bytes;
		if(all_bytes >= UID_SHOW_DAILY_LIMIT)
			show_io_uid_tasks(m, uid_entry);
	}
	rt_mutex_unlock(&oppo_uid_lock);
	return 0;
}
...
static int uidio_show_open(struct inode *inode, struct file *file)
{
	...
	do_each_thread(temp, task) {
		...
		if (!uid_entry)
			continue;
		add_uid_io_stats(uid_entry, task, UID_STATE_TOTAL_CURR, task->state);
...
struct proc_dir_entry *create_uid_proc(struct proc_dir_entry *parent)
{
	...
	dir_entry = proc_create("uid_io_show", S_IRUGO | S_IWUGO, parent, &proc_uidio_show_operations);
