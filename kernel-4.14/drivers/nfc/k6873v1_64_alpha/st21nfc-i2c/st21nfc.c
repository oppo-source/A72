					...
					int mode)
{
	...
	ret = devm_request_irq(dev, client->irq, st21nfc_dev_irq_handler,
			       st21nfc_dev->polarity_mode, client->name,
			       st21nfc_dev);
	if (ret) {
...
#define PREX "Error transition from"
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
	"%s Idle state to Active state with notification: 0x%llx\n",
	    PREX, data->c_pw_states_err.idle_to_active_ntf);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
"%s Active Reader/Writer state to Active Reader/Writer state: 0x%llx\n",
	    PREX, data->c_pw_states_err.act_rw_to_act_rw);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
"%s Idle state to Active Reader/Writer state with notification: 0x%llx\n",
	    PREX, data->c_pw_states_err.idle_to_active_rw_ntf);
 ...
 *	    "0x%llx\n"
 *	    "Error transition from Active state to Active state: 0x%llx\n"
 *	    "Error transition from Idle state to Active state with "
 *	    "notification: "
 *	    "0x%llx\n"
 *	    "Error transition from Active Reader/Writer state to Active "
 *	    "Reader/Writer state: 0x%llx\n"
 *	    "Error transition from Idle state to Active Reader/Writer state "
 *	    "with "
 *	    "notification: 0x%llx\n"
 *	    "\nTotal uptime: 0x%llx Cumulative modes time: 0x%llx\n",
...
static struct attribute *st21nfc_attrs[] = {
	&dev_attr_i2c_addr.attr, &dev_attr_version.attr,
	&dev_attr_power_stats.attr, NULL,
};
			 ...
			 const struct i2c_device_id *id)
{
...
#ifndef ST21NFCD_MTK
	ret =
	    acpi_dev_add_driver_gpios(ACPI_COMPANION(dev), acpi_st21nfc_gpios);
	if (ret)
	...
	if (!np) {
		pr_info("%s : cannot find mediatek,nfc-gpio-v2 in DTS.\n",
		       __func__);
		return -ENODEV;
	}
#endif // ST21NFCD_MTK
	...
	if (!gpio_is_valid(r)) {
		pr_info("%s: get NFC IRQ GPIO failed (%d)", __FILE__, r);
		return -ENODEV;
	}
	...
	if (ret) {
		pr_info("%s : gpio_request failed\n", __FILE__);
		return -ENODEV;
	}
	pr_info("%s : IRQ GPIO = %d\n", __func__, r);
	ret = gpio_direction_input(r);
	if (ret) {
		pr_info("%s : gpio_direction_input failed\n", __FILE__);
		return -ENODEV;
	}
#endif // ST21NFCD_MTK
	...
	if (!gpio_is_valid(r)) {
		pr_info("%s: get NFC RST GPIO failed (%d)", __FILE__, r);
		return -ENODEV;
	}
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
		pr_info("%s : Unable to request reset-gpios\n", __func__);
		return -ENODEV;
	}
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		pr_info("[OPTIONAL] %s: Unable to request pidle-gpio\n",
			__func__);
		ret = 0;
	} else {
	...
	if (IS_ERR(st21nfc_dev->gpiod_clkreq)) {
		pr_info("[OPTIONAL] %s : Unable to request clkreq-gpios\n",
			__func__);
		ret = 0;
	} else {
		...
		if (ret < 0) {
			pr_info("%s : st21nfc_clock_select failed\n", __func__);
	...
	pr_debug(
	    ...
	    client->irq,
	    st21nfc_dev->gpiod_pidle ? desc_to_gpio(st21nfc_dev->gpiod_pidle)
				     : -1);
