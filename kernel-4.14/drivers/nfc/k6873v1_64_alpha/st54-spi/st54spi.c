			   ...
			   unsigned int n_xfers)
{
	...
	for (n = n_xfers, u_tmp = u_xfers; n; n--, u_tmp++) {
		if (u_tmp->rx_buf) {
			if (__copy_to_user(
				(u8 __user *)(uintptr_t)u_tmp->rx_buf, rx_buf,
				u_tmp->len)) {
				status = -EFAULT;
				goto done;
			}
			  ...
			  unsigned long arg)
{
	...
	case SPI_IOC_RD_MODE:
		retval =
		    __put_user(spi->mode & SPI_MODE_MASK, (__u8 __user *)arg);
		break;
	case SPI_IOC_RD_MODE32:
		retval =
		    __put_user(spi->mode & SPI_MODE_MASK, (__u32 __user *)arg);
		break;
	...
	case ST54SPI_IOC_RD_POWER:
		retval =
		    __put_user(st54spi_power_get(st54spi), (__u32 __user *)arg);
		break;
...
static int st54spi_release(struct inode *inode, struct file *filp)
{
	...
	if (!st54spi->users) {
		...
		dofree =
		    ((st54spi->spi == NULL) && (st54spi->spi_reset == NULL));
		spin_unlock_irq(&st54spi->spi_lock);
...
static int st54spi_parse_dt(struct device *dev, struct st54spi_data *pdata)
{
	...
	if (np) {
		...
		if (!power_mode) {
			...
			pr_info("%s: Power mode unknown: %s\n", __FILE__,
			       power_mode);
			return -1;
		}
	} else {
...
static int st54spi_probe(struct spi_device *spi)
{
	...
	if (minor < N_SPI_MINORS) {
		...
		dev =
		    device_create(st54spi_class, &spi->dev, st54spi->devt,
				  //    spidev, "spidev%d.%d",
	...
	if (st54spi->power_gpio != 0) {
		if (st54spi->power_gpio_mode == POWER_MODE_ST54J_COMBO)
			default_value = 1;
...
#else
		pr_info("%s : st54spi as module cannot use ST54H fully\n",
		       __func__);
#endif
	}
