...
void mt_print_dbg_register(void)
{
	int i;

	for (i = 0; i <= 0x500; i+=0x4) {
		if (clk_readl(rb[topckgen].virt + i) == 0)
			continue;
		pr_info("%s: [0x%08x] = 0x%08x\n",
			__func__, rb[topckgen].phys + i, clk_readl(rb[topckgen].virt + i));
	}

	for (i = 0; i <= 0x410; i+=0x4) {
		if (clk_readl(rb[apmixed].virt + i) == 0)
			continue;
		pr_info("%s: [0x%08x] = 0x%08x\n",
			__func__, rb[apmixed].phys + i, clk_readl(rb[apmixed].virt + i));
	}
}

extern bool get_is_scp_bootup(void);

static bool mt_get_fmeter_cond(void)
{
#ifdef CONFIG_MTK_TINYSYS_SCP_SUPPORT
	struct timespec kernel_time;
	unsigned long val;

	get_monotonic_boottime(&kernel_time);
	val = (unsigned long)kernel_time.tv_sec;

	if (!get_is_scp_bootup() && val < 15)
		return false;
