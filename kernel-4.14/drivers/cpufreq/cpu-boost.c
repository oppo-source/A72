				...
				void *data)
{
	...
	unsigned int clu = topology_physical_package_id(cpu);

	unsigned int ppm_min = (ib_min) ? ib_min : UINT_MAX;
	mt_ppm_sysboost_set_freq_limit(BOOST_BY_CPU_BOOST, clu, ppm_min, UINT_MAX);
	pr_debug("CLU%u ppm  boost: %u kHz\n", clu, ib_min);
...
static void do_input_boost(struct work_struct *work)
{
	...
	if (sched_boost_on_input > 0) {
		ret = set_sched_boost(sched_boost_on_input);
		if (ret)
