...
void himax_limit_read(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	pr_info("himax_test_limit_show:array_limit_offset = %x array_limitcbc_offset = %x\n",
		ph->array_limit_offset, ph->array_limitcbc_offset);
	TPD_DEBUG("test begin:\n");
	seq_puts(s, "Without cbc:");
...
static const struct file_operations himax_proc_reset_ops = {
	.owner = THIS_MODULE, .write = himax_proc_reset_write,
};
...
static const struct file_operations himax_proc_sense_on_off_ops = {
	.owner = THIS_MODULE, .write = himax_proc_sense_on_off_write,
};
		      ...
		      struct himax_proc_operations *syna_ops)
{
	...
	if (prEntry_himax == NULL) {
		ret = -ENOMEM;
		pr_info("%s: Couldn't create /proc/touchpanel/debug_info/himax proc entry\n",
			 __func__);
	}
