 ...
 * Revision 1.1, 2017-04-17, MingQiang.Guo@Bsp.Group.Tp, modify based on gerrit
 *review result(http://gerrit.scm.adc.com:8080/#/c/326176)
 ****************************************************************/
...
#define CUSTOM_O17001TRULY_TD4322_1080P_CMD_PANEL 29
...
//#ifdef CONFIG_VMAP_STACK
#if 0
	result = spi_write_then_read(private_ts->s_client,
		&spi_format_buf[0], 3, data, length);
#else
...
static int himax_bus_write(uint8_t command, uint32_t length, uint8_t *data)
{
	...
	if (spi_format_buf == NULL) {
		spi_format_buf =
		    kzalloc((alloc_size + 2) * sizeof(uint8_t), GFP_KERNEL);
	}
...
static int hx83112f_mode_switch(void *chip_data, enum work_mode mode,
	bool flag);
static uint32_t himax_hw_check_CRC(uint8_t *start_addr, int reload_length);
static enum fw_check_state hx83112f_fw_check(void *chip_data,
			  ...
			  uint8_t *write_data, bool cfg_flag)
{
	...
	if (cfg_flag == false) {
		...
		for (i = address; i < address + write_length; i++) {
			...
			himax_flash_write_burst_length(write_addr, write_data,
						       write_length);
		}
	} else if (cfg_flag == true) {
		if (himax_bus_write(write_addr[0], write_length, write_data) <
		    0) {
			TPD_INFO("%s: i2c access fail!\n", __func__);
			return;
		}
				    ...
				    uint8_t *write_data, uint8_t cfg_flag)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_bus_size) {
			memcpy(tmp_data, write_data + (i * max_bus_size),
			       max_bus_size);
		...
		if (tmp_addr[0] < write_addr[0]) {
			...
			tmp_addr[1] =
			    write_addr[1] + (uint8_t)((address >> 8) & 0x00FF);
		}
...
#ifdef HX_RST_PIN_FUNC
		hx83112f_resetgpio_set(g_chip_info->hw_res,
					       false); // reset gpio
		hx83112f_resetgpio_set(g_chip_info->hw_res,
					       true); // reset gpio
#else
...
void himax_diag_register_set(uint8_t diag_command)
{
	...
	himax_register_read(tmp_addr, 4, tmp_data, false);
	pr_info("%s: tmp_data[3] = 0x%02X, tmp_data[2] = 0x%02X, tmp_data[1] = 0x%02X, tmp_data[0] = 0x%02X!\n",
...
bool wait_wip(int Timing)
{
	...
	do {
		...
		if (in_buffer[0] != 0x00 || in_buffer[1] != 0x00 ||
			...
			pr_info("%s:Wait wip retry_cnt:%d, buffer[0]=%d, buffer[1]=%d, buffer[2]=%d, buffer[3]=%d\n",
 ...
 */
static int hx83112f_enable_interrupt(struct chip_data_hx83112f *chip_info,
				     bool enable)
{
...
void himax_in_parse_assign_cmd(uint32_t addr, uint8_t *cmd, int len)
{
	...
	case 4:
		...
		/*  TPD_INFO("%s: cmd[0] = 0x%02X,cmd[1] = 0x%02X,cmd[2] =
		 *  0x%02X,cmd[3] = 0x%02X\n",
...
int hx_dis_rload_0f(int disable)
{
	...
	do {
		...
		himax_register_read(pzf_op->addr_dis_flash_reload, 4, tmp_data,
				    false);
		pr_info("Now data: tmp_data[3] = 0x%02X || tmp_data[2] = 0x%02X || tmp_data[1] = 0x%02X || tmp_data[0] = 0x%02X\n",
			   tmp_data[3], tmp_data[2], tmp_data[1], tmp_data[0]);
		if (tmp_data[3] != 0x00 || tmp_data[2] != 0x00 ||
		    tmp_data[1] != 0x9A || tmp_data[0] != 0xA9) {
			pr_info("Now data: tmp_data[3] = 0x%02X || tmp_data[2] = 0x%02X || tmp_data[1] = 0x%02X || tmp_data[0] = 0x%02X\n",
				 tmp_data[3], tmp_data[2], tmp_data[1],
				 tmp_data[0]);
			TPD_INFO(
			    "Not Same,Write Fail, there is %d retry times!\n",
			    retry);
		} else {
			check_val = 1;
...
void himax_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	tmp_addr[1] = addr[1];
	tmp_addr[0] = addr[0];
	pr_info("%s, write addr tmp_addr[3] = 0x%2.2X, tmp_addr[2] = 0x%2.2X, tmp_addr[1] = 0x%2.2X, tmp_addr[0] = 0x%2.2X\n",
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			...
			TPD_DETAIL("last total_size_temp=%d\n",
				   total_size_temp);
			himax_flash_write_burst_length(
			    tmp_addr, tmp_data, total_size_temp % max_bus_size);
		}
		address = ((i + 1) * max_bus_size);
			     ...
			     int start_index, uint32_t write_len)
{
	...
	if (g_chip_info->tmp_data == NULL) {
		...
		// __func__);
		g_chip_info->tmp_data = kcalloc(
		    firmware_update_space, sizeof(uint8_t), GFP_KERNEL);
		if (g_chip_info->tmp_data == NULL) {
	...
	for (i = 0; i < (total_read_times); i++) {
		 ...
		 *TPD_INFO("[log]addr[3] = 0x%02X, addr[2] = 0x%02X, addr[1] =
		 *0x%02X, addr[0] = 0x%02X!\n", tmp_addr[3], tmp_addr[2],
		...
		if (total_size_temp >= max_bus_size) {
			himax_flash_write_burst_length(
			    tmp_addr,
			    &(g_chip_info
				  ->tmp_data[start_index + i * max_bus_size]),
			    max_bus_size);
			total_size_temp = total_size_temp - max_bus_size;
		} else {
			TPD_DETAIL("last total_size_temp=%d\n",
...
bool hx_parse_bin_cfg_data(const struct firmware *fw_entry)
{
	...
	/*0. check 1k header*/
	if (fw_entry->data[0x00] == 0x00 && fw_entry->data[0x01] == 0x00 &&
	    fw_entry->data[0x02] == 0x00 && fw_entry->data[0x03] == 0x00 &&
	    fw_entry->data[0x04] == 0x00 && fw_entry->data[0x05] == 0x00 &&
	    fw_entry->data[0x06] == 0x00 && fw_entry->data[0x07] == 0x00 &&
	...
	for (i = 0; i < part_num; i++) {
		/*3. get all partition*/
		if (flag_1k_header == true)
			memcpy(buf, &fw_entry->data[i * 0x10 + HX64K + HX1K],
			       16);
		else
	...
	hx83112f_nf_cfg_sz =
	    (dsram_max - dsram_base) + zf_info_arr[i_max].write_size;
	hx83112f_nf_cfg_sz = hx83112f_nf_cfg_sz + (hx83112f_nf_cfg_sz % 16);

	TPD_INFO(
	    "%s, hx83112f_nf_cfg_sz = %d!, dsram_base = %X, dsram_max = %X\n",
	    __func__, hx83112f_nf_cfg_sz, dsram_base, dsram_max);

	if (hx83112f_nf_FW_buf == NULL)
		hx83112f_nf_FW_buf =
		    kzalloc(sizeof(unsigned char) * FW_BIN_16K_SZ, GFP_KERNEL);

	for (i = 1; i < part_num; i++)
		memcpy(hx83112f_nf_FW_buf +
			   (zf_info_arr[i].cfg_addr - dsram_base),
		       (unsigned char *)&fw_entry->data[zf_info_arr[i].fw_addr],
		       zf_info_arr[i].write_size);

	hx83112f_nf_cfg_crc = himax_mcu_Calculate_CRC_with_AP(
	    hx83112f_nf_FW_buf, 0, hx83112f_nf_cfg_sz);
...
static int hx83112f_nf_zf_part_info(const struct firmware *fw_entry)
{
	...
	if (flag_1k_header == true)
		himax_sram_write_crc_check(
		    fw_entry, pzf_op->data_sram_start_addr, HX1K, HX64K);
	else
		himax_sram_write_crc_check(
		    fw_entry, pzf_op->data_sram_start_addr, 0, HX64K);
	crc = himax_hw_check_CRC(pzf_op->data_sram_start_addr, HX64K);
	...
	do {
		himax_mcu_register_write(hx83112f_nf_sram_min,
					 hx83112f_nf_cfg_sz, hx83112f_nf_FW_buf,
					 0);
		/*himax_register_write(hx83112f_nf_sram_min, hx83112f_nf_cfg_sz,
		 * hx83112f_nf_FW_buf, 0);
		 */
		crc = himax_hw_check_CRC(hx83112f_nf_sram_min,
					 hx83112f_nf_cfg_sz);
		if (crc != hx83112f_nf_cfg_crc)
			pr_info("Config CRC FAIL, HW CRC = %X, SW CRC = %X, retry time = %d",
...
fw_reload:
		...
		if (g_chip_info->g_fw_sta) {
			...
			if (g_chip_info->p_firmware_headfile->firmware_data) {
				request_fw_headfile->size =
				    g_chip_info->p_firmware_headfile
					->firmware_size;
	...
	if ((int)tmp_fw_entry->size > HX64K) {
		...
		/* first 64K */
		do {
			g_core_fp.fp_write_sram_0f(tmp_fw_entry,
						   pzf_op->data_sram_start_addr,
						   0, HX64K);
			crc = himax_hw_check_CRC(pzf_op->data_sram_start_addr,
						 HX64K);
			...
			pr_info("%s, 64K LAST 4 BYTES: data[3] = 0x%02X, data[2] = 0x%02X, data[1] = 0x%02X, data[0] = 0x%02X\n",
				   __func__, temp_data[3], temp_data[2],
		    ...
		    ((temp_data[3] == 0 && temp_data[2] == 0 &&
		      temp_data[1] == 0 && temp_data[0] == 0 && retry < 80) ||
		     (crc != 0 && retry < 30)) &&
		    !(g_chip_info->using_headfile && retry < 3));
		...
		if (G_POWERONOF == 1) {
			retry = 0;
			do {
				g_core_fp.fp_write_sram_0f(
				    tmp_fw_entry, pzf_op->data_cfg_info, 0xC000,
				    128); // 132
				crc = himax_hw_check_CRC(pzf_op->data_cfg_info,
							 128);
				if (crc == 0) {
					TPD_DETAIL(
					    "%s, config info ok in %d time\n",
					    __func__, retry);
					break;
				}
				...
				retry++;
			} while ((crc != 0 && retry < 30) ||
				 (g_chip_info->using_headfile && retry < 15));
		...
		if (G_POWERONOF == 1) {
			...
			do {
				g_core_fp.fp_write_sram_0f(
				    tmp_fw_entry, pzf_op->data_fw_cfg_p1,
				    0xC100, 528); // 482
				crc = himax_hw_check_CRC(pzf_op->data_fw_cfg_p1,
							 528);
				if (crc == 0) {
					TPD_DETAIL(
					    "%s, 1 FW config ok in %d time\n",
					    __func__, retry);
					break;
				}
			...
			g_core_fp.fp_clean_sram_0f(pzf_op->data_fw_cfg_p1, 528,
						   1);
		}
		...
		if (G_POWERONOF == 1) {
			...
			do {
				g_core_fp.fp_write_sram_0f(
				    tmp_fw_entry, pzf_op->data_fw_cfg_p3,
				    0xCA00, 128);
				crc = himax_hw_check_CRC(pzf_op->data_fw_cfg_p3,
							 128);
				if (crc == 0) {
					TPD_DETAIL(
					    "%s, 3 FW config ok in %d time\n",
					    __func__, retry);
					break;
				}
				pr_info("%s, 3 FW config fail in %d time !\n",
						 __func__, retry);
			...
			g_core_fp.fp_clean_sram_0f(pzf_op->data_fw_cfg_p3, 128,
						   1);
		}
		...
		if (G_POWERONOF == 1) {
			...
			do {
				g_core_fp.fp_write_sram_0f(
				    tmp_fw_entry, pzf_op->data_adc_cfg_1,
				    0xD640, 1200);
				crc = himax_hw_check_CRC(pzf_op->data_adc_cfg_1,
							 1200);
				if (crc == 0) {
					TPD_DETAIL(
					    "%s, 1 ADC config ok in %d time\n",
					    __func__, retry);
					break;
				}
				pr_info("%s, 1 ADC config fail in %d time !\n",
						 __func__, retry);
		...
		if (G_POWERONOF == 1) {
			...
			do {
				g_core_fp.fp_write_sram_0f(
				    tmp_fw_entry, pzf_op->data_adc_cfg_2,
				    0xD320, 800);
				crc = himax_hw_check_CRC(pzf_op->data_adc_cfg_2,
							 800);
				if (crc == 0) {
					TPD_DETAIL(
					    "%s, 2 ADC config ok in %d time\n",
					    __func__, retry);
					break;
				}
				pr_info("%s, 2 ADC config fail in %d time !\n",
						 __func__, retry);
		...
		if (G_POWERONOF == 1) {
			...
			do {
				g_core_fp.fp_write_sram_0f(
				    tmp_fw_entry, pzf_op->data_map_table,
				    0xE000, 1536);
				crc = himax_hw_check_CRC(pzf_op->data_map_table,
							 1536);
				if (crc == 0) {
					pr_info("%s, mapping table ok in %d time\n",
						   __func__, retry);
					break;
				}
				pr_info("%s, mapping table fail in %d time !\n",
						 __func__, retry);
	...
	if (G_POWERONOF == 1) {
		g_core_fp.fp_write_sram_0f(tmp_fw_entry,
					   pzf_op->data_mode_switch, 0xC30C, 4);
	} else {
		g_core_fp.fp_clean_sram_0f(pzf_op->data_mode_switch, 4, 2);
	}

	hx83112f_fw_check(private_ts->chip_data, &private_ts->resolution_info,
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (private_ts->fw_update_app_support) {
		err = request_firmware_select(&fw_entry, file_name,
					      private_ts->dev);
	} else {
...
/* trigger reset */
	...
	if (g_chip_info->debug_mode_sta) {
		if (private_ts->apk_op && private_ts->apk_op->apk_debug_set) {
			private_ts->apk_op->apk_debug_set(private_ts->chip_data,
							  true);
		}
	}
			    ...
			    int start_index, int read_len)
{
	...
	tmp_addr[1] = addr[1];
	tmp_addr[0] = addr[0];
	pr_info("%s, read addr tmp_addr[3] = 0x%2.2X, tmp_addr[2] = 0x%2.2X, tmp_addr[1] = 0x%2.2X, tmp_addr[0] = 0x%2.2X\n",
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_bus_size) {
			himax_register_read(tmp_addr, max_bus_size,
...
void himax_mcu_read_all_sram(uint8_t *addr, int read_len)
{
	...
	pr_info("%s, NOW addr tmp_addr[3] = 0x%2.2X, tmp_addr[2] = 0x%2.2X, tmp_addr[1] = 0x%2.2X, tmp_addr[0] = 0x%2.2X\n",
		 __func__, tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0]);
...
void himax_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	...
	if (type == 0) { /* first 64K */
		...
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_cfg_info,
					  0xC000, 132);
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_fw_cfg, 0xC0FE,
					  512);
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_adc_cfg_1,
					  0xD000, 376);
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_adc_cfg_2,
					  0xD178, 376);
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_adc_cfg_3,
					  0xD000, 376);
...
void himax_mcu_0f_operation_check(int type)
{
	...
	TPD_INFO("first 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 g_chip_info->g_fw_buf[0], g_chip_info->g_fw_buf[1],
		 g_chip_info->g_fw_buf[2], g_chip_info->g_fw_buf[3]);
	TPD_INFO("next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 g_chip_info->g_fw_buf[4], g_chip_info->g_fw_buf[5],
		 g_chip_info->g_fw_buf[6], g_chip_info->g_fw_buf[7]);
	TPD_INFO("and next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 g_chip_info->g_fw_buf[8], g_chip_info->g_fw_buf[9],
		 g_chip_info->g_fw_buf[10], g_chip_info->g_fw_buf[11]);
...
bool himax_ic_package_check(void)
{
	...
	for (i = 0; i < 5; i++) {
		 ...
		 * ((tmp_data[1] == 0x2f) || (tmp_data[1] == 0x2f)))
		 */
		if (1) {
...
static void himax_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		cmd[3] = 0x10; // custom_o fw id bin address : 0xc014   , 49172
			       // Tp ic address : 0x 10007014
		cmd[2] = 0x00;
		cmd[1] = 0x7f;
		cmd[0] = 0x00;
	...
	pr_info("%s : data[0] = 0x%2.2X, data[1] = 0x%2.2X, data[2] = 0x%2.2X, data[3] = 0x%2.2X\n",
	...
	//=====================================
	// Read FW version : 0x1000_7004  but 05,06 are the real addr for FW
	// Version
	//=====================================
...
void himax_read_CUSTOM_O_FW_ver(struct chip_data_hx83112f *chip_info)
{
	uint8_t cmd[4];
	uint8_t data[4];
	uint32_t touch_ver = 0;

	cmd[3] = 0x10; // custom_o fw id bin address : 0xc014 Tp ic address : 0x
		       // 10007014
	cmd[2] = 0x00;
	cmd[1] = 0x70;
	cmd[0] = 0x14;
...
uint32_t himax_hw_check_CRC(uint8_t *start_addr, int reload_length)
{
	...
	tmp_addr[1] = 0x00;
	tmp_addr[0] = 0x20;
	// tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = 0xFB;
	// tmp_data[0] = 0x00;
	...
	do {
		...
		if ((tmp_data[0] & 0x01) != 0x01) {
			...
			tmp_addr[0] = 0x18;
			himax_register_read(tmp_addr, 4, tmp_data, false);
			pr_info("%s: tmp_data[3]=%X, tmp_data[2]=%X, tmp_data[1]=%X, tmp_data[0]=%X\n",
...
int himax_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00) { // more than 4 fingers
		hx_touch_data->rawdata_size =
		    cal_data_len(hx_touch_data->raw_cnt_rmd, max_touch_point,
				 hx_touch_data->raw_cnt_max);
		hx_touch_data->touch_info_size =
		    (max_touch_point + hx_touch_data->raw_cnt_max + 2) * 4;
	} else { // less than 4 fingers
		hx_touch_data->rawdata_size =
		    cal_data_len(hx_touch_data->raw_cnt_rmd, max_touch_point,
				 hx_touch_data->raw_cnt_max);
		hx_touch_data->touch_info_size =
		    (max_touch_point + hx_touch_data->raw_cnt_max + 1) * 4;
	}

	if ((tx_num * rx_num + tx_num + rx_num) % hx_touch_data->rawdata_size ==
	    0) {
		hx_touch_data->rawdata_frame_size =
		    (tx_num * rx_num + tx_num + rx_num) /
		    hx_touch_data->rawdata_size;
	} else {
		hx_touch_data->rawdata_frame_size =
		    (tx_num * rx_num + tx_num + rx_num) /
	...
	TPD_INFO("%s: rawdata_frame_size = %d ", __func__,
		 hx_touch_data->rawdata_frame_size);
	pr_info("%s: max_touch_point:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d, g_hx_rawdata_size:%d, hx_touch_data->touch_info_size:%d\n",
...
void himax_esd_hw_reset(struct chip_data_hx83112f *chip_info)
{
	...
	do {
		...
		TPD_DETAIL(
		    "It will update fw after esd event in zero flash mode!\n");
...
int himax_checksum_cal(struct chip_data_hx83112f *chip_info, uint8_t *buf,
		       int ts_status)
{
		...
		if (ts_status == HX_REPORT_COORD) {
			...
			if (buf[loop_i] == 0xEB) {
				...
				hx_ED_event++;
				   ...
				   0x00) { /* case 2 ESD recovery flow-Disable
					    */
				hx_zero_event++;
			} else {
				hx_EB_event = 0;
			...
			if (hx_EB_event == length) {
				hx_esd_event = length;
				hx_EB_event_flag++;
				pr_info("[HIMAX TP MSG]: ESD event checked - ALL 0xEB.\n");
			} else if (hx_EC_event == length) {
				hx_esd_event = length;
				hx_EC_event_flag++;
				pr_info("[HIMAX TP MSG]: ESD event checked - ALL 0xEC.\n");
	...
	if (ts_status == HX_REPORT_COORD) {
		...
		if (hx_esd_event == length || hx_zero_event == length) {
			...
			if (shaking_ret == CHECKSUM_FAIL) {
				...
				// TPD_INFO("I2C running. Nothing to be
				// done!\n");
				goto workqueue_out;
			}
			...
			pr_info("[HX_ESD_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				 __func__);
			goto checksum_fail;
		} else if (HX_HW_RESET_ACTIVATE) {
			/* drop 1st interrupts after chip reset */
			HX_HW_RESET_ACTIVATE = 0;
			pr_info("[HX_HW_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				 __func__);
			goto ready_to_serve;
		}
	}
	...
	if ((check_sum_cal % 0x100 != 0)) {
		TPD_INFO(
		    "[HIMAX TP MSG] checksum fail : check_sum_cal: 0x%02X\n",
		    check_sum_cal);
		// goto checksum_fail;
		goto workqueue_out;
	}
...
void himax_idle_mode(int disable)
{
	...
	do {
		...
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s:After turn ON/OFF IDLE Mode [0] = 0x%02X, [1] = 0x%02X, [2] = 0x%02X, [3] = 0x%02X\n",
...
int hx_test_data_pop_out(struct chip_data_hx83112f *chip_info,
			 ...
			 char *g_project_test_info_log, char *rslt_buf,
			 char *filepath)
{
	struct file *raw_file = NULL;
	struct filename *vts_name = NULL;
	char *line = "=================================================\n";
	char *Company = "Himax for CUSTOM_O: Driver Seneor Test\n";
	char *Info = "Test Info as follow\n";
	char *project_name_log = "CUSTOM_O_";
	mm_segment_t fs;
	loff_t pos = 0;
	int ret_val = NO_ERR;
	...
	/*Company Info*/
	snprintf(g_Company_info_log, 160, "%s%s%s%s", line, Company, Info,
		 line);
	...
	snprintf(g_project_test_info_log, 118,
		...
		get_project(),
		 ...
		 19131,
#endif
		 chip_info->fw_id, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
		 chip_info->hw_res->RX_NUM);
	// snprintf(g_project_test_info_log, 118, "Project_name: %s%d\nFW_ID:
	// %8X\nFW_Ver: %4X\nPanel Info: TX_Num=%d RX_Num=%d\nTest stage:
	// Mobile\n",
	//         project_name_log, 12345, chip_info->fw_id, chip_info->fw_ver,
...
int hx_test_data_get(struct chip_data_hx83112f *chip_info, uint32_t RAW[],
		     char *start_log, char *result, int now_item)
{
	uint32_t i;
...
int himax_get_rawdata(struct chip_data_hx83112f *chip_info, uint32_t *RAW,
		      uint32_t datalen)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	while (retry < 10) {
		...
		for (i = 2; i < total_size; i = i + 2) {
			checksum_cal +=
			    tmp_rawdata[i + 1] * 256 + tmp_rawdata[i];
		}
	...
	// 4 Copy Data
	for (i = 0; i < chip_info->hw_res->RX_NUM * chip_info->hw_res->TX_NUM;
	     i++) {
	...
	for (i = 0; i < chip_info->hw_res->RX_NUM; i++) {
		pr_info("TX%2d", i + 1);
		for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
			// if ((j == SKIPRXNUM) && (i >= SKIPTXNUM_START) && (i
			// <= SKIPTXNUM_END)) {
			// continue;
			//} else {
			index = ((chip_info->hw_res->RX_NUM *
				      chip_info->hw_res->TX_NUM -
				  i) -
				 chip_info->hw_res->RX_NUM * j) -
				1;

			pr_info("%6d", RAW[index]);
...
uint32_t himax_check_mode(uint8_t checktype)
{
	...
	himax_register_read(tmp_addr, 4, tmp_data, false);
	TPD_INFO(
	    "%s: himax_wait_sorting_mode, tmp_data[0]=%x, tmp_data[1]=%x\n",
	    __func__, tmp_data[0], tmp_data[1]);
...
void himax_get_noise_base(void)
{
	...
	/*TPD_INFO("tmp_data[0]=0x%x tmp_data[1]=0x%x tmp_data[2]=0x%x
	 *   tmp_data[3]=0x%x\n",
...
uint32_t himax_wait_sorting_mode(uint8_t checktype)
{
	...
	do {
		...
		tmp_addr[0] = 0x04;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: tmp_data[0]=%x, tmp_data[1]=%x\n",
			 __func__, tmp_data[0], tmp_data[1]);
		...
		tmp_addr[0] = 0xA8;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 0x900000A8, tmp_data[0]=%x, tmp_data[1]=%x, tmp_data[2]=%x, tmp_data[3]=%x\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
		...
		tmp_addr[0] = 0xE4;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 0x900000E4, tmp_data[0]=%x, tmp_data[1]=%x, tmp_data[2]=%x, tmp_data[3]=%x\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
		...
		tmp_addr[0] = 0xF8;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 0x900000F8, tmp_data[0]=%x, tmp_data[1]=%x, tmp_data[2]=%x, tmp_data[3]=%x\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
...
int mpTestFunc(struct chip_data_hx83112f *chip_info, uint8_t checktype,
	       uint32_t datalen)
{
	uint8_t tmp_addr[4] = { 0 };
	uint8_t tmp_data[4] = { 0 };
	...
	if (himax_check_mode(checktype)) {
		TPD_INFO("Need Change Mode, target=%s",
			 g_himax_inspection_mode[checktype]);
	...
	if (ret) {
		...
		tmp_addr[0] = 0xA8;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 900000A8: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X,\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
		...
		tmp_addr[0] = 0x40;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 10007F40: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X,\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
		...
		tmp_addr[0] = 0x00;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 10000000: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X,\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
		...
		tmp_addr[0] = 0x04;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 10007F04: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X,\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
		...
		tmp_addr[0] = 0xEC;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s: 800204B4: data[0]=%0x02X, data[1]=%0x02X, data[2]=%0x02X, data[3]=%0x02X,\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
	...
	switch (checktype) {
	case HIMAX_INSPECTION_OPEN:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
				 chip_info->hw_res->RX_NUM);
		     i++) {
			...
			if (isRead_csv == false) {
				...
				if (RAW[i] > hx83112f_nf_inspection_criteria
						 [IDX_OPENMAX][i] ||
				    RAW[i] < hx83112f_nf_inspection_criteria
						 [IDX_OPENMIN][i]) {
					TPD_INFO("%s: open test FAIL\n",
						 __func__);
					ret = RESULT_ERR;
	...
	case HIMAX_INSPECTION_RAWDATA:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (isRead_csv == false) {
				if (RAW[i] > CRITERIA_RAWDATA_MAX ||
				    RAW[i] < CRITERIA_RAWDATA_MIN) {
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (isRead_csv == false) {
				if (RAW[i] > CRITERIA_LPWUG_RAWDATA_MAX ||
				    RAW[i] < LPWUG_RAWDATA_MIN) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_RAWDATA FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ERAW;
				}
			} else {
				if (RAW[i] > hx83112f_nf_inspection_criteria
						 [IDX_LPWUG_RAWDATA_MAX][i] ||
				    RAW[i] < hx83112f_nf_inspection_criteria
						 [IDX_LPWUG_RAWDATA_MIN][i]) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_RAWDATA FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ERAW;
				}
	...
	case HIMAX_INSPECTION_LPWUG_NOISE:
		if (isBD12proj || isEA006proj) {
			for (i = 0; i < (chip_info->hw_res->TX_NUM *
				...
				if (isRead_csv == false) {
					if (RAW[i] > LPWUG_NOISE_MAX ||
					    RAW[i] < LPWUG_NOISE_MIN) {
						pr_info("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n",
							 __func__);
						ret = THP_AFE_INSPECT_ENOISE;
					}
				} else {
					if (RAW[i] >
						hx83112f_nf_inspection_criteria
						    [IDX_LPWUG_NOISE_MAX][i] ||
					    RAW[i] <
						hx83112f_nf_inspection_criteria
						    [IDX_LPWUG_NOISE_MIN][i]) {
						pr_info("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n",
							 __func__);
						ret = THP_AFE_INSPECT_ENOISE;
					}
			...
			if (isRead_csv == false) {
				...
				if (weight > LPWUG_NOISEMAX ||
				    weight < hx83112f_nf_inspection_criteria
						 [IDX_LPWUG_NOISE_MIN][i]) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ENOISE;
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (isRead_csv == false) {
				if (RAW[i] > CRITERIA_LPWUG_IDLE_RAWDATA_MAX ||
				    RAW[i] < LPWUG_IDLE_RAWDATA_MIN) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ERAW;
				}
			} else {
				if (RAW[i] >
					hx83112f_nf_inspection_criteria
					    [IDX_LPWUG_IDLE_RAWDATA_MAX][i] ||
				    RAW[i] <
					hx83112f_nf_inspection_criteria
					    [IDX_LPWUG_IDLE_RAWDATA_MIN][i]) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ERAW;
				}
	...
	case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (isRead_csv == false) {
				if (RAW[i] > LPWUG_IDLE_NOISE_MAX ||
				    RAW[i] < LPWUG_IDLE_NOISE_MIN) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ENOISE;
				}
			} else {
				if (RAW[i] >
					hx83112f_nf_inspection_criteria
					    [IDX_LPWUG_IDLE_NOISE_MAX][i] ||
				    RAW[i] <
					hx83112f_nf_inspection_criteria
					    [IDX_LPWUG_IDLE_NOISE_MIN][i]) {
					pr_info("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE FAIL\n",
						 __func__);
					ret = THP_AFE_INSPECT_ENOISE;
				}
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		...
		if (hx83112f_nf_inspt_crtra_flag[i] == 1) {
			for (j = 0; j < all_mut_len; j++) {
				TPD_DEBUG(
				    "%d, ",
				    hx83112f_nf_inspection_criteria[i][j]);
				if (j % 16 == 15)
					TPD_DEBUG("\n");
			}
		} else {
...
static int hx_get_crtra_by_name(struct chip_data_hx83112f *chip_info,
				char **result, int size_of_result_str)
{
	int i = 0;
	...
	for (i = 0; i < size_of_result_str && result[i] != NULL; i++) {
		...
		if (temp != -9487) {
			hx83112f_nf_inspection_criteria[count_type]
						       [count_data] = temp;
		} else {
...
static int himax_parse_criteria_file(struct chip_data_hx83112f *chip_info)
{
	...
	err = request_firmware(&file_entry, chip_info->test_limit_name,
			       private_ts->dev);
	if (err < 0) {
...
static void hx83112f_black_screen_test(void *chip_data, char *message)
{
	...
	struct chip_data_hx83112f *chip_info =
	    (struct chip_data_hx83112f *)chip_data;
	int i = 0;
	...
	/*Init Log Data */
	g_1kind_raw_size =
	    5 * chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM * 2;
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_LPWUG_RAWDATA,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	...
	snprintf(buf, 128, "6. MP_LPWUG_TEST_RAW: %s\n",
		 error ? "Error" : "Ok");
	hx83112f_nf_fail_write_count += snprintf(
	    g_Test_list_log + hx83112f_nf_fail_write_count, 15, "test Item:\n");
	hx83112f_nf_fail_write_count +=
	    snprintf(g_Test_list_log + hx83112f_nf_fail_write_count, 32,
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_LPWUG_NOISE,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_LPWUG_IDLE_NOISE,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	sprintf(g_file_name_OK,
		"tp_testlimit_gesture_OK_%02d%02d%02d-%02d%02d%02d-utc.csv",
		(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1,
		rtc_now_time.tm_mday, rtc_now_time.tm_hour, rtc_now_time.tm_min,
		rtc_now_time.tm_sec);
	sprintf(g_file_name_NG,
		"tp_testlimit_gesture_NG_%02d%02d%02d-%02d%02d%02d-utc.csv",
		(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1,
		rtc_now_time.tm_mday, rtc_now_time.tm_hour, rtc_now_time.tm_min,
		rtc_now_time.tm_sec);

	hx83112f_nf_fail_write_count +=
	    snprintf(g_Test_list_log + hx83112f_nf_fail_write_count, 22,
		     "Final_result: %s\n", error_num ? "Fail" : "Pass");
	if (error) {
		snprintf(g_file_path_NG, (int)(strlen(HX_GES_RSLT_OUT_PATH_NG) +
					       strlen(g_file_name_NG) + 1),
		...
		snprintf(g_file_path_OK, (int)(strlen(HX_GES_RSLT_OUT_PATH_OK) +
					       strlen(g_file_name_OK) + 1),
...
RET_OUT:
	if (hx83112f_nf_inspection_criteria != NULL) {
		...
		hx83112f_nf_inspection_criteria = NULL;
		TPD_INFO(
		    "Now it have free the hx83112f_nf_inspection_criteria!\n");
	} else {
			 ...
			 char *g_Test_list_log)
{
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_OPEN,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_MICRO_OPEN,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_SHORT,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_RAWDATA,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		error = mpTestFunc(
		    chip_info, HIMAX_INSPECTION_NOISE,
		    (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM) +
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
...
void himax_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size;
		...
		tmp_data[1] = 0xF0;
		tmp_data[0] = 0x00;
		// data bytes should be 0x6100_0000 + ((word_number)*4-1)*4096 =
		// 0x6100_0000 + 0xFF000 = 0x610F_F000
		// Programmable size = 1 page = 256 bytes, word_number = 256
		// byte / 4 = 64
		himax_flash_write_burst(tmp_addr, tmp_data);
		...
		// tmp_data[0] = 0x00; // Flash start address 1st : 0x0000_0000
		...
		for (j = 0; j < 5; j++) {
			...
			if (himax_bus_write(0x00, program_length + 4,
					    buring_data) < 0) {
				TPD_INFO("%s: i2c access fail!\n", __func__);
				goto RET_OUT;
			}
...
int fts_ctpm_fw_upgrade_with_sys_fs_64k(unsigned char *fw, int len,
					bool change_iref) /* Alice - Un */
{
	return 0;
}

static size_t hx83112f_proc_register_read(struct file *file, char *buf,
					  size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		himax_register_read(register_command, max_bus_size, data,
				    cfg_flag);
...
static size_t hx83112f_proc_register_write(struct file *file, const char *buff,
					   size_t len, loff_t *pos)
{
	char buf[81] = { 0 };
	char buf_tmp[16];
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' &&
		...
		if (buf[0] == 'r') {
			...
			if (count < 3) {
				...
				himax_register_write(register_command,
						     byte_length, w_data,
						     cfg_flag);
			} else {
			 ...
			 uint8_t lb)
{
	...
	do {
		...
		msleep(20);
		himax_register_read(tmp_addr, 4, tmp_data, false);
		pr_info("%s:Now tmp_data[0] = 0x%02X, [1] = 0x%02X, [2] = 0x%02X, [3] = 0x%02X\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3]);
	} while ((tmp_data[1] != hb && tmp_data[0] != lb) && cnt++ < 100);

	if (cnt >= 99) {
		pr_info("%s ERR Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n",
			__func__, tmp_addr[3], tmp_data[1], tmp_data[0]);
		return -1;
	}

	TPD_INFO(
	    "Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n",
...
void himax_get_DSRAM_data(uint8_t *info_data, uint8_t x_num, uint8_t y_num)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_i2c_size) {
			himax_register_read(tmp_addr, max_i2c_size,
	...
	for (i = 2; i < total_size; i = i + 2) { /* 2:PASSWORD NOT included */
		...
		pr_info("0x%2x:0x%4x ", temp_info_data[i], check_sum_cal);
		if (i % 32 == 0)
			pr_info("\n");
	}
...
void himax_ts_diag_func(struct chip_data_hx83112f *chip_info,
			int32_t *mutual_data)
{
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
		for (j = 0; j < chip_info->hw_res->RX_NUM; j++) {
			new_data = ((int8_t)info_data[index + 1] << 8 |
				    info_data[index]);
			mutual_data[i * chip_info->hw_res->RX_NUM + j] =
			    new_data;
			index += 2;
		}
	}
			 ...
			 int32_t *self_data)
{
	...
	if (hx_touch_data->hx_rawdata_buf[0] == 0x3A &&
		...
		index =
		    (hx_touch_data->hx_rawdata_buf[2] - 1) * RawDataLen_word;
		// TPD_INFO("Header[%d]: %x, %x, %x, %x, mutual: %d, self:
		// %d\n", index, buf[56], buf[57], buf[58], buf[59], mul_num,
		// self_num);
		// TPD_INFO("RawDataLen=%d , RawDataLen_word=%d ,
		// hx_touch_info_size=%d\n", RawDataLen, RawDataLen_word,
		// hx_touch_info_size);
...
static size_t hx83112f_proc_diag_write(struct file *file, const char *buff,
				       size_t len, loff_t *pos)
{
	char messages[80] = { 0 };
	...
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	struct chip_data_hx83112f *chip_info =
	    (struct chip_data_hx83112f *)ts->chip_data;

	/* 0: common, other: dsram*/
	int storage_type = 0;
	/* 1:IIR, 2:DC, 3:Bank, 4:IIR2, 5:IIR2_N, 6:FIR2, 7:Baseline, 8:dump
	 * coord
	 */
	int rawdata_type = 0;
	...
	if (g_diag_command > 0 && rawdata_type == 0) {
		pr_info("[Himax]g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d! Maybe no support!\n",
			 g_diag_command, storage_type, rawdata_type);
		g_diag_command = 0x00;
	} else {
		pr_info("[Himax]g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d\n",
			 g_diag_command, storage_type, rawdata_type);
	}
...
static size_t hx83112f_proc_diag_read(struct file *file, char *buff, size_t len,
				      loff_t *pos)
{
	size_t ret = 0;
	char *temp_buf;
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		// if ((g_diag_command >= 1 && g_diag_command <= 7) ||
		// dsram_type > 0)
		{
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM *
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx_touch_data->diag_mutual[i] >
				    g_max_mutual) {
					g_max_mutual =
					    hx_touch_data->diag_mutual[i];
				}
				if (hx_touch_data->diag_mutual[i] <
				    g_min_mutual) {
					g_min_mutual =
...
uint8_t himax_read_DD_status(struct chip_data_hx83112f *chip_info,
			     uint8_t *cmd_set, uint8_t *tmp_data)
{
	int cnt = 0;
	uint8_t req_size = cmd_set[0];
	uint8_t cmd_addr[4] = { 0xFC, 0x00, 0x00,
				0x90 }; // 0x900000FC -> cmd and hand shaking
	uint8_t tmp_addr[4] = { 0x80, 0x7F, 0x00,
				0x10 }; // 0x10007F80 -> data space

	cmd_set[3] = 0xAA;
	himax_register_write(cmd_addr, 4, cmd_set, 0);

	pr_info("%s: cmd_set[0] = 0x%02X, cmd_set[1] = 0x%02X, cmd_set[2] = 0x%02X, cmd_set[3] = 0x%02X\n",
		 __func__, cmd_set[0], cmd_set[1], cmd_set[2], cmd_set[3]);

	for (cnt = 0; cnt < 100; cnt++) {
		himax_register_read(cmd_addr, 4, tmp_data, false);
		pr_info("%s: tmp_data[0] = 0x%02X, tmp_data[1] = 0x%02X, tmp_data[2] = 0x%02X, tmp_data[3] = 0x%02X, cnt=%d\n",
			 __func__, tmp_data[0], tmp_data[1], tmp_data[2],
			 tmp_data[3], cnt);
		usleep_range(10000, 10001);
		if (tmp_data[3] == 0xBB) {
...
static size_t hx83112f_proc_DD_debug_read(struct file *file, char *buf,
					  size_t len, loff_t *pos)
{
	int ret = 0;
	uint8_t tmp_data[64];
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (mutual_set_flag == 1) {
			if (himax_read_DD_status(chip_info, cmd_set,
						 tmp_data) == NO_ERR) {
...
static size_t hx83112f_proc_DD_debug_write(struct file *file, const char *buff,
					   size_t len, loff_t *pos)
{
	uint8_t i = 0;
	uint8_t cnt = 2;
	...
	if (buf_tmp[2] == 'x' && buf_tmp[6] == 'x' && buf_tmp[10] == 'x') {
		...
		for (i = 3; i < 12; i = i + 4) {
			...
			if (!kstrtoul(buf_tmp2, 16, &result))
				...
				pr_info("String to oul is fail in cnt = %d,buf_tmp2 = %s",
					 cnt, buf_tmp2);
			cnt--;
		}
		TPD_INFO(
		    "cmd_set[2] = %02X, cmd_set[1] = %02X, cmd_set[0] = %02X\n",
...
int himax_read_FW_status(struct chip_data_hx83112f *chip_info,
					  ...
					  size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		cmd_set[0] = 0x01;
		if (himax_read_FW_status(chip_info, cmd_set, tmp_data) ==
		    NO_ERR) {
			ret += snprintf(temp_buf + ret, len - ret,
		...
		if (himax_read_FW_status(chip_info, cmd_set, tmp_data) ==
			...
			for (loop_i = 0; loop_i < cmd_set[1];
				...
				ret +=
				    snprintf(temp_buf + ret, len - ret, "%5d\t",
					     tmp_data[loop_i] +
						 (tmp_data[loop_i + 1] << 8));
				if ((loop_i % 16) == 14)
					ret += snprintf(temp_buf + ret,
...
static int hx83112f_configuration_init(struct chip_data_hx83112f *chip_info,
				       bool config)
{
	int ret = 0;
...
int himax_ic_reset(struct chip_data_hx83112f *chip_info, uint8_t loadconfig,
		   uint8_t int_off)
{
	int ret = 0;
	...
	if (chip_info->hw_res->reset_gpio) {
		if (int_off) {
			...
			if (ret < 0) {
				TPD_INFO(
				    "%s: hx83112f enable interrupt failed.\n",
				    __func__);
				return ret;
			}
		}
		...
		if (loadconfig) {
			...
			if (ret < 0) {
				TPD_INFO(
				    "%s: hx83112f configuration init failed.\n",
				    __func__);
				return ret;
			}
...
static size_t hx83112f_proc_sense_on_off_write(struct file *file,
...
static int hx83112f_get_touch_points(void *chip_data, struct point_info *points,
				     int max_num)
{
	...
	if (g_diag_command)
		...
		ret =
		    himax_read_event_stack(buf, hx_touch_data->touch_info_size);
	if (!ret) {
	...
	check_sum_cal =
	    himax_checksum_cal(chip_info, buf, ts_status); //????checksum
	...
	if (g_diag_command) {
		...
		self_num =
		    chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
		pr_info("hx_touch_data->touch_all_size= %d hx_touch_data->touch_info_size = %d, %d\n",
		...
		diag_parse_raw_data(hx_touch_data, mutual_num, self_num,
				    g_diag_command, hx_touch_data->diag_mutual,
				    diag_self);
	}

	if (hx_touch_data->hx_coord_buf[HX_TOUCH_INFO_POINT_CNT] == 0xff)
		// HX_TOUCH_INFO_POINT_CNT buf???????
		hx_point_num = 0;
	else
		hx_point_num =
	...
	for (i = 0; i < 10; i++) {
		x = hx_touch_data->hx_coord_buf[i * 4] << 8 |
		    hx_touch_data->hx_coord_buf[i * 4 + 1];
		y = (hx_touch_data->hx_coord_buf[i * 4 + 2] << 8 |
		     hx_touch_data->hx_coord_buf[i * 4 + 3]);
		z = hx_touch_data->hx_coord_buf[i + 40];
		if (x >= 0 && x <= private_ts->resolution_info.max_x &&
		    y >= 0 && y <= private_ts->resolution_info.max_y) {
			points[i].x = x;
			points[i].y = y;
...
static int hx83112f_get_vendor(void *chip_data, struct panel_info *panel_data)
{
	...
	pr_info("chip_info->tp_type = %d, panel_data->test_limit_name = %s, panel_data->fw_name = %s\n",
		 chip_info->tp_type, panel_data->test_limit_name,
		 panel_data->fw_name);
	return 0;
}

static int hx83112f_get_chip_info(void *chip_data) { return 1; }
...
static uint32_t hx83112f_get_fw_id(struct chip_data_hx83112f *chip_info)
{
	...
	cmd[3] = 0x10; // custom_o fw id bin address : 0xc014   , 49172    Tp ic
		       // address : 0x 10007014
	cmd[2] = 0x00;
	cmd[1] = 0x70;
	cmd[0] = 0x14;
...
static u8 hx83112f_trigger_reason(void *chip_data, int gesture_enable,
				  int is_suspended)
{
 ...
 *    //struct chip_data_hx83112f *chip_info = (struct chip_data_hx83112f
 **)chip_data;
 *
 *    TPD_INFO("%s.\n", __func__);
 *    //hx83112f_resetgpio_set(chip_info->hw_res, true); // reset gpio
...
static int hx83112f_reset(void *chip_data)
{
	...
	if (!chip_info->first_download_finished) {
		TPD_INFO(
		    "%s:First download has not finished, don't do reset.\n",
		    __func__);
		return 0;
	}
	...
	/*Yulianghan@RM.PSW.BSP.TP, 2018/09/07, add for hx83112f_noflash lcd esd
	 * TP irq exception in realme 18612
	 */
...
void himax_ultra_enter(void)
{
	...
	do {
		...
		pr_info("%s:retry times %d, addr = 0x34, correct 0x11 = current 0x%2.2X\n",
	...
	do {
		...
		pr_info("%s:retry times %d, addr = 0x33, correct 0x33 = current 0x%2.2X\n",
			 __func__, rtimes, tmp_data[0]);
		rtimes++;
	...
	do {
		...
		pr_info("%s:retry times %d, addr = 0x34, correct 0x22 = current 0x%2.2X\n",
			 __func__, rtimes, tmp_data[0]);
		rtimes++;
	...
	do {
		...
		pr_info("%s:retry times %d, addr = 0x33, correct 0xAA = current 0x%2.2X\n",
			 __func__, rtimes, tmp_data[0]);
		rtimes++;
...
static int hx83112f_enable_black_gesture(struct chip_data_hx83112f *chip_info,
					 bool enable)
{
	int ret = 0;
	...
	if (ts->is_suspended) {
		if (enable) {
#ifdef HX_RST_PIN_FUNC
			hx83112f_resetgpio_set(chip_info->hw_res,
					       true); // reset gpio
			hx83112f_resetgpio_set(chip_info->hw_res,
					       false); // reset gpio
			hx83112f_resetgpio_set(chip_info->hw_res,
			...
			// himax_sense_on(0);
			/*if (!HX_RESET_STATE) {
			 *    ret =  hx83112f_resetgpio_set(chip_info->hw_res,
			 *true); // reset gpio
			 *    if (ret < 0) {
...
/*
 *static int hx83112f_enable_edge_limit(struct chip_data_hx83112f *chip_info,
 *	bool enable)
 *{
 *    int ret = 0;
 *    return ret;
...
static int hx83112f_enable_charge_mode(struct chip_data_hx83112f *chip_info,
				       bool enable)
{
	int ret = 0;
	uint8_t tmp_addr[4];
...
static int hx83112f_jitter_switch(struct chip_data_hx83112f *chip_info, bool on)
{
	...
	if (!on) { // jitter off
		do {
			if (rtimes > 10) {
				TPD_INFO(
				    "%s:retry over 10, jitter off failed!\n",
				    __func__);
				pr_info("%s:correct tmp_data[0, 1, 2, 3] = 0x5A, 0xA5, 0x5A, 0xA5\n",
					 __func__);
				ret = -1;
				break;
			}
			...
			pr_info("%s:retry times %d, current tmp_data[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
				 __func__, rtimes, tmp_data[0], tmp_data[1],
		...
		do {
			if (rtimes > 10) {
				TPD_INFO(
				    "%s:retry over 10, jitter on failed!\n",
				    __func__);
				pr_info("%s:correct tmp_data[0, 1, 2, 3] = 0x00, 0x00, 0x00, 0x00\n",
					 __func__);
				ret = -1;
				break;
			}
...
static int hx83112f_enable_headset_mode(struct chip_data_hx83112f *chip_info,
					bool enable)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	if (ts->headset_pump_support) {
		if (enable) { /* insert headset */
			do {
				...
				himax_register_read(tmp_addr, 4, tmp_data,
						    false);
				pr_info("%s:retry times %d, current tmp_data[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
					   __func__, rtimes, tmp_data[0],
...
static int hx83112f_rotative_switch(struct chip_data_hx83112f *chip_info,
				    int mode)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	if (ts->fw_edge_limit_support) {
		if (mode == 1 ||
		    VERTICAL_SCREEN ==
			chip_info->touch_direction) { /* vertical */
			do {
				if (rtimes > 10) {
			...
			rtimes = 0;
			if (LANDSCAPE_SCREEN_270 ==
			    chip_info->touch_direction) { // turn right
				do {
					if (rtimes > 10) {
						pr_info("%s:rotative right failed!\n",
							 __func__);
						pr_info("%s:correct tmp_data[0, 1, 2, 3] = 0x3A, 0xA3, 0x3A, 0xA3\n",
							 __func__);
						ret = -1;
						break;
					}
					...
					tmp_data[1] = 0xA3;
					tmp_data[0] = 0x3A;
					himax_flash_write_burst(tmp_addr,
								tmp_data);
			...
			} else if (LANDSCAPE_SCREEN_90 ==
				   chip_info->touch_direction) { // turn left
				do {
					if (rtimes > 10) {
						pr_info("%s:rotative left failed!\n",
							 __func__);
						pr_info("%s:correct tmp_data[0, 1, 2, 3] = 0x1A, 0xA1, 0x1A, 0xA1\n",
							 __func__);
						ret = -1;
						break;
					}
		...
		if (mode) { // open
			...
			do {
				if (rtimes > 10) {
					pr_info("%s:close edge limit failed!\n",
					    __func__);
					pr_info("%s:correct tmp_data[0, 1, 2, 3] = 0x9A, 0xA9, 0x9A, 0xA9\n",
						 __func__);
					ret = -1;
					break;
				}
...
static int hx83112f_mode_switch(void *chip_data, enum work_mode mode, bool flag)
{
	int ret = -1;
	struct chip_data_hx83112f *chip_info =
	    (struct chip_data_hx83112f *)chip_data;
	...
	case MODE_EDGE:
		...
		if (ret < 0) {
			TPD_INFO(
			    "%s: hx83112f enable edg & corner limit failed.\n",
			    __func__);
			return ret;
		}
...
static int hx83112f_get_gesture_info(void *chip_data,
	...
	for (i = 0; i < 4; i++) {
		if (check_FC == 0) {
			if ((buf[0] != 0x00) && ((buf[0] < 0x0E))) {
				...
				check_FC = 0;
				// TPD_DEBUG("ID START at %x , value = %x skip
				// the event\n", i, buf[i]);
				break;
			}
		} else {
			if (buf[i] != gesture_sign) {
				check_FC = 0;
				// TPD_DEBUG("ID NOT the same %x != %x So STOP
				// parse event\n", buf[i], gesture_sign);
				break;
			}
		}
		...
		if (check_point_format == 0) {
			...
			while (i < (gest_len + 1) / 2) {
				...
				if (i == 6) {
					...
					gest_pt_x[gest_pt_cnt] =
					    buf[GEST_PTLG_ID_LEN + 4 + i * 2] *
					    private_ts->resolution_info.max_x /
					    255;
				}
			...
			if (private_ts->gesture_buf) {
				...
				if (private_ts->gesture_buf[0] == 0x07) {
					for (j = 0; j < gest_len * 2;
					     j = j + 2) {
						private_ts->gesture_buf[3 + j] =
						    buf[n];
						private_ts
						    ->gesture_buf[3 + j + 1] =
						    buf[n + 1];
					...
					memcpy(&private_ts->gesture_buf[3],
					       &buf[24], 80);
				}
			}
		}
				 ...
				 char *g_Test_list_log)
{
	...
	if (eint_count == 10) {
		...
		hx83112f_nf_fail_write_count +=
		    snprintf(g_Test_list_log + hx83112f_nf_fail_write_count, 45,
			     "eint_status is low, TP EINT direct stort,\n");
		// store_to_file(syna_testdata->fd, "eint_status is low, TP EINT
		// direct stort, \n");
		eint_count = 0;
		return TEST_FAIL;
	}
...
static void hx83112f_auto_test(struct seq_file *s, void *chip_data,
			       struct syna_testdata *syna_testdata)
{
	int error_count = 0;
	int ret = THP_AFE_INSPECT_OK;
	...
	usleep_range(5000, 5001);
	himax_sense_on(0x00);
	himax_read_CUSTOM_O_FW_ver(chip_info);

	ret = hx83112f_enable_interrupt(chip_info, false);

	error_count +=
	    hx83112f_int_pin_test(s, chip_info, syna_testdata, g_Test_list_log);
	...
	rtc_time_to_tm(now_time.tv_sec, &rtc_now_time);
	sprintf(g_file_name_OK,
		"tp_testlimit_OK_%02d%02d%02d-%02d%02d%02d-utc.csv",
		(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1,
		rtc_now_time.tm_mday, rtc_now_time.tm_hour, rtc_now_time.tm_min,
		rtc_now_time.tm_sec);
	sprintf(g_file_name_NG,
		"tp_testlimit_NG_%02d%02d%02d-%02d%02d%02d-utc.csv",
		(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1,
		rtc_now_time.tm_mday, rtc_now_time.tm_hour, rtc_now_time.tm_min,
		rtc_now_time.tm_sec);

	if (error_count) {
		snprintf(g_file_path_NG, (int)(strlen(HX_RSLT_OUT_PATH_NG) +
					       strlen(g_file_name_NG) + 1),
		...
		snprintf(g_file_path_OK, (int)(strlen(HX_RSLT_OUT_PATH_OK) +
					       strlen(g_file_name_OK) + 1),
...
static void hx83112f_read_debug_data(struct seq_file *s, void *chip_data,
				     int debug_data_type)
{
	uint16_t mutual_num;
	uint16_t self_num;
	...
	data_mutual_sram =
	    kzalloc(chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM *
			sizeof(int32_t),
		    GFP_KERNEL);
	if (!data_mutual_sram)
		goto RET_OUT;
	...
	for (j = 0; j < chip_info->hw_res->RX_NUM; j++) {
		for (i = 0; i < chip_info->hw_res->TX_NUM; i++) {
			k = ((mutual_num - j) - chip_info->hw_res->RX_NUM * i) -
			    1;
...
static enum fw_update_state hx83112f_fw_update(void *chip_data,
					  const struct firmware *fw, bool force)
{
	...
	// step 2:Get FW version from IC && determine whether we need get into
	// update flow.
	...
	// fts_ctpm_fw_upgrade_with_sys_fs_64k((unsigned char *)fw->data,
	// fw->size, false);
...
static int hx83112f_get_usb_state(void) { return upmu_get_rgs_chrdet(); }
...
static struct custom_o_touchpanel_operations hx83112f_ops = {
...
static int himax_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:HIMAX%06X\nFW_VER:0x%04X\nCH:%dX%d\n",
		       0x83112F, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
		       chip_info->hw_res->RX_NUM);
	if (ret > len)
		ret = len;
...
static void himax_init_custom_o_apk_op(struct touchpanel_data *ts)
{
...

{
	...
	memcpy(&chip_info->hx_spi_mcc, &hx_spi_ctrdata,
	...
	chip_info->himax_0f_update_wq =
	    create_singlethread_workqueue("HMX_0f_update_reuquest");
...
// msecs_to_jiffies(2000));
	...
	if (ts->boot_mode == RECOVERY_BOOT) {
		...
		queue_delayed_work(chip_info->himax_0f_update_wq,
				   &chip_info->work_0f_update,
...
#ifdef CONFIG_FB
	.suspend = hx83112f_i2c_suspend, .resume = hx83112f_i2c_resume,
#endif
};
