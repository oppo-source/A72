 ...
 * Returning transfer length(transfer success) or most likely negative
 * errno(transfer error)
 */
			 ...
			 unsigned short length, unsigned char *data)
{
	...
	if (retry == MAX_I2C_RETRY_TIME) {
		pr_info((&client->dev, "%s: I2C read over retry limit\n",
			__func__);
		retval = -EIO;
	}
	return retval;
			 ...
			 unsigned short length, unsigned char *data)
{
	...
	static unsigned int read_buf_size;
	...
	if (length > FIX_I2C_LENGTH) {
		if (read_buf_size < length) {
			...
			if (!read_buf) {
				read_buf_size = 0;
				pr_info("read block_1, kzalloc failed(len:%d,buf_size:%d).\n",
					 length, read_buf_size);
				mutex_unlock(&i2c_mutex);
				return -ENOMEM;
			}
			read_buf_size = length;
			pr_info("read block_1, kzalloc success(len:%d,buf_size:%d).\n",
				 length, read_buf_size);
		} else {
		...
		if (read_buf_size > FIX_I2C_LENGTH) {
			...
			if (!read_buf) {
				read_buf_size = 0;
				pr_info("read block_2, kzalloc failed(len:%d,buf_size:%d).\n",
					 length, read_buf_size);
				mutex_unlock(&i2c_mutex);
				return -ENOMEM;
			}
			read_buf_size = FIX_I2C_LENGTH;
			pr_info("read block_2, kzalloc success(len:%d,buf_size:%d).\n",
				 length, read_buf_size);
		} else {
			if (!read_buf) {
				...
				if (!read_buf) {
					read_buf_size = 0;
					pr_info("read block_3, kzalloc failed(len:%d,buf_size: %d).\n ",
						length, read_buf_size);
					    mutex_unlock(&i2c_mutex);
					return -ENOMEM;
				}
			  ...
			  unsigned short length, unsigned char const *data)
{
	...
	static unsigned int write_buf_size;
	...
	if (total_length > FIX_I2C_LENGTH) {
		...
		"write block_1, kzalloc failed(len:%d,buf_size: %d).\n",
			...
			pr_info("write block_1, kzalloc success(len:%d,buf_size:%d).\n",
				 total_length, write_buf_size);
		} else {
		...
		"write block_2, kzalloc failed(len:%d,buf_size: %d).\n",
			...
			pr_info("write block_2, kzalloc success(len:%d,buf_size:%d).\n",
				 total_length, write_buf_size);
		} else {
			if (!write_buf) {
				...
				if (!write_buf) {
					write_buf_size = 0;
					pr_info("write block_3, kzalloc failed(len:%d,buf_size: %d).\n",
						total_length, write_buf_size);
					    mutex_unlock(&i2c_mutex);
					return -ENOMEM;
				}
 ...
 * Returning zero(transfer success) or most likely negative errno(transfer
 * error)
 */
 ...
 * Returning negative errno else a 16-bit unsigned "word" received from the
 * device.
 */
...
/**
 * touch_i2c_read - Using for "read data from ic after writing or not" through
 * IIC
 * @client: Handle to slave device
 ...
 *
 * Actully, This function call i2c_transfer for IIC transfer,
 * Returning transfer msg length(transfer success) or most likely negative
 * errno(transfer EIO error)
 */
 ...
 * @flag_register_16bit: bool param to detect whether this device using 16bit
 * IIC address or 8bit address
 *
 * Actully, This function don't have many operation, we just detect device
 * address length && alloc DMA memory for MTK platform
 * Returning zero(success) or -ENOMEM(memory alloc failed)
 */
...
int32_t CTP_SPI_READ(struct spi_device *client, uint8_t *buf, uint16_t len)
{
	...
	//    TPD_INFO("rbuf[SPI_READ_LEN + 1]=rbuf[%d], len+DUMMY_BYTES =
	//    %d\n",
	//    SPI_READ_LEN + 1, len + DUMMY_BYTES);
	while (retries < 5) {
