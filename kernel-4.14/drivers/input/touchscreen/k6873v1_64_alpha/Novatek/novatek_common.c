			      ...
			      size_t count, loff_t *offp)
{
	...
	if (i2c_wr == 0) { // I2C write
		...
		while (retries < 20) {
			addr_tmp = ts->client->addr;
			ts->client->addr = str[0] & 0x7F;
			ret = touch_i2c_read(ts->client, &str[2], 1, &str[3],
					     str[1] - 1);
			ts->client->addr = addr_tmp;
			if (ret == 2)
				break;
...
static const struct file_operations nvt_flash_fops = {
	.owner = THIS_MODULE, .open = simple_open, .read = nvt_flash_read,
};
				...
				size_t count, loff_t *offp)
{
	...
	str = kzalloc((count), GFP_KERNEL | GFP_DMA);
	if (str == NULL) {
...
static const struct file_operations nvt_noflash_fops = {
	.owner = THIS_MODULE, .open = simple_open, .read = nvt_noflash_read,
};
...
void nvt_flash_proc_init(struct touchpanel_data *ts, const char *name)
{
	...
	if (strstr(name, "SPI")) {
		...
		nvt_proc_entry =
		    proc_create_data(name, 0444, NULL, &nvt_noflash_fops, ts);
		...
		nvt_proc_entry =
		    proc_create_data(name, 0444, NULL, &nvt_flash_fops, ts);
		if (nvt_proc_entry == NULL) {
			TPD_INFO("%s Failed!\n", __func__);
			return;
		}
...
void nvt_limit_read(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	open_rawdata_p =
	    (int32_t *)(fw->data + ph->array_open_rawdata_P_offset);
	open_rawdata_n =
	    (int32_t *)(fw->data + ph->array_open_rawdata_N_offset);
	short_rawdata_p =
	    (int32_t *)(fw->data + ph->array_Short_Rawdata_P_offset);
	short_rawdata_n =
	    (int32_t *)(fw->data + ph->array_Short_Rawdata_N_offset);
	...
	doze_rawdata_p =
	    (int32_t *)(fw->data + ph->array_Doze_Rawdata_P_offset);
	doze_rawdata_n =
	    (int32_t *)(fw->data + ph->array_Doze_Rawdata_N_offset);
	doze_diff_rawdata_p =
	    (int32_t *)(fw->data + ph->array_Doze_Diff_P_offset);
	doze_diff_rawdata_n =
	    (int32_t *)(fw->data + ph->array_Doze_Diff_N_offset);
	lpwg_rawdata_p =
	...
	for (i = 0; i < ts->hw_res.RX_NUM; i++) {
		...
		for (j = 0; j < ts->hw_res.TX_NUM; j++) {
			seq_printf(s, "%4d, ",
				   fw_rawdata_p[i * ts->hw_res.TX_NUM + j]);
		}
	}
	...
	for (i = 0; i < ts->hw_res.RX_NUM; i++) {
		...
		for (j = 0; j < ts->hw_res.TX_NUM; j++) {
			seq_printf(s, "%4d, ",
				   fw_rawdata_n[i * ts->hw_res.TX_NUM + j]);
		}
	}
	...
	for (i = 0; i < ts->hw_res.RX_NUM; i++) {
		...
		for (j = 0; j < ts->hw_res.TX_NUM; j++) {
			seq_printf(s, "%4d, ",
				   open_rawdata_p[i * ts->hw_res.TX_NUM + j]);
		}
	}
	...
	for (i = 0; i < ts->hw_res.RX_NUM; i++) {
		...
		for (j = 0; j < ts->hw_res.TX_NUM; j++) {
			seq_printf(s, "%4d, ",
				   open_rawdata_n[i * ts->hw_res.TX_NUM + j]);
		}
	}

	if (ph->config_Lmt_Short_Rawdata_P != 0) {
		seq_printf(s, "\n config_Lmt_Short_Rawdata_P: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    short_rawdata_p[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_Short_Rawdata_N != 0) {
		seq_printf(s, "\n config_Lmt_Short_Rawdata_N: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    short_rawdata_n[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	seq_printf(s, "\n config_Diff_Test_Frame: %4d",
		   ph->config_Diff_Test_Frame);

	if (ph->config_Lmt_FW_Diff_P != 0) {
		seq_printf(s, "\n config_Lmt_FW_Diff_P: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    diff_rawdata_p[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_FW_Diff_N != 0) {
		seq_printf(s, "\n config_Lmt_FW_Diff_N: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    diff_rawdata_n[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_FW_CC_P != 0) {
		seq_printf(s, "\n config_Lmt_FW_CC_P: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    cc_data_p[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_FW_CC_N != 0) {
		seq_printf(s, "\n config_Lmt_FW_CC_N: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    cc_data_n[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}
	...
	if (ph->config_Lmt_Doze_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->doze_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    doze_rawdata_p[i * ph->doze_X_Channel + j]);
			}
		}
	}

	if (ph->config_Lmt_Doze_Rawdata_N != 0) {
		seq_printf(s, "\n config_Lmt_Doze_Rawdata_N: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->doze_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    doze_rawdata_n[i * ph->doze_X_Channel + j]);
			}
		}
	}

	seq_printf(s, "\n config_Doze_Noise_Test_Frame: %4d",
		   ph->config_Doze_Noise_Test_Frame);

	if (ph->config_Lmt_Doze_Diff_P != 0) {
		seq_printf(s, "\n config_Lmt_Doze_Diff_P: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->doze_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    doze_diff_rawdata_p[i * ph->doze_X_Channel +
							j]);
			}
		}
	}
	...
	if (ph->config_Lmt_Doze_Diff_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->doze_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    doze_diff_rawdata_n[i * ph->doze_X_Channel +
							j]);
			}
		}
	}
	...
	if (ph->config_Lmt_LPWG_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    lpwg_rawdata_p[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_LPWG_Rawdata_N != 0) {
		seq_printf(s, "\n config_Lmt_LPWG_Rawdata_N: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    lpwg_rawdata_n[i * ts->hw_res.TX_NUM + j]);
			}
		}
	}

	if (ph->config_Lmt_LPWG_Diff_P != 0) {
		seq_printf(s, "\n config_Lmt_LPWG_Diff_P: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    lpwg_diff_rawdata_p[i * ts->hw_res.TX_NUM +
							j]);
			}
		}
	}
	...
	if (ph->config_Lmt_LPWG_Diff_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ts->hw_res.TX_NUM; j++) {
				seq_printf(
				    s, "%4d, ",
				    lpwg_diff_rawdata_n[i * ts->hw_res.TX_NUM +
							j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FDM_Rawdata_P != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->fdm_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    fdm_rawdata_p[i * ph->fdm_X_Channel + j]);
			}
		}
	}

	if (ph->config_Lmt_FDM_Rawdata_N != 0) {
		seq_printf(s, "\n config_Lmt_FDM_Rawdata_N: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->fdm_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    fdm_rawdata_n[i * ph->fdm_X_Channel + j]);
			}
		}
	}

	seq_printf(s, "\n config_FDM_Noise_Test_Frame: %4d",
		   ph->config_FDM_Noise_Test_Frame);

	if (ph->config_Lmt_FDM_Diff_P != 0) {
		seq_printf(s, "\n config_Lmt_FDM_Diff_P: %4d",
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->fdm_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    fdm_diff_rawdata_p[i * ph->fdm_X_Channel +
						       j]);
			}
		}
	}
	...
	if (ph->config_Lmt_FDM_Diff_N != 0) {
		...
		for (i = 0; i < ts->hw_res.RX_NUM; i++) {
			...
			for (j = 0; j < ph->fdm_X_Channel; j++) {
				seq_printf(
				    s, "%4d, ",
				    fdm_diff_rawdata_n[i * ph->fdm_X_Channel +
						       j]);
			}
		}
	}
		    ...
		    struct nvt_proc_operations *nvt_ops)
{
	...
	prEntry_tmp = proc_create_data("baseline_test", 0644, ts->prEntry_tp,
