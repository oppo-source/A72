/******************************************************************
 * Copyright (c)  2008- 2019  CUSTOM_O Mobile communication Corp.ltd.
 * VENDOR_EDIT
...
/****************** Start of Log Tag Declear and level
 * define*******************************/
...
/******************** End of Log Tag Declear and level
 * define*********************************/
...
static enum fw_update_state nvt_fw_update_sub(void *chip_data,
					 const struct firmware *fw, bool force);
static enum fw_update_state nvt_fw_update(void *chip_data,
	const struct firmware *fw, bool force);
static int nvt_reset(void *chip_data);
static int nvt_get_chip_info(void *chip_data);

static struct chip_data_nt36672c *g_chip_info;
...
static const struct mtk_chip_config spi_ctrdata = {
	.rx_mlsb = 1, .tx_mlsb = 1, .cs_pol = 0, .cs_setuptime = 25,
};
#else
...
static int nvt_esd_handle(void *chip_data)
{
	struct chip_data_nt36672c *chip_info =
	    (struct chip_data_nt36672c *)chip_data;
	...
	for (retry = 5; retry > 0; retry--) {
		...
		for (list = 0; list < (sizeof(trim_id_table) /
			...
			if (found_nvt_chip) {
				...
				chip_info->trim_id_table.mmap =
				    trim_id_table[list].mmap;
				chip_info->trim_id_table.carrier_system =
				    trim_id_table[list].carrier_system;
				chip_info->trim_id_table.support_hw_crc =
				    trim_id_table[list].support_hw_crc;
				ret = 0;
				goto out;
			} else {
 ...
 ********************************************************/
static int32_t
nvt_check_fw_reset_state_noflash(struct chip_data_nt36672c *chip_info,
				 enum RST_COMPLETE_STATE check_reset_state)
{
	...
	while (1) {
		...
		if (unlikely(retry > retry_max)) {
			pr_info("error, retry=%d, buf[1]=0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
...
static int nvt_enter_sleep(struct chip_data_nt36672c *chip_info, bool config)
{
	...
	if (config) {
		...
		TPD_INFO("%s: enter sleep mode success!\n", __func__);
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		if (retry_count < 3) {
			...
			pr_info("Set default fw_ver=0, x_num=18, y_num=32, abs_x_max=1080, abs_y_max=1920, max_button_num=0!\n");
			ret = -1;
		}
	} else {
		ret = 0;
				     ...
				     const u8 *fwdata, size_t fwsize)
{
	...
	tmp_end = fwdata[0] + (fwdata[1] << 8) + (fwdata[2] << 16) +
		  (fwdata[3] << 24);
	 ...
	 */
	chip_info->partition =
	    chip_info->ilm_dlm_num + ovly_sec_num + info_sec_num;
	...
	for (list = 0; list < chip_info->partition; list++) {
		...
		if ((list >= chip_info->ilm_dlm_num) &&
			...
			chip_info->bin_map[list].SRAM_addr =
			    byte_to_word(&fwdata[pos]);
			chip_info->bin_map[list].size =
			    byte_to_word(&fwdata[pos + 4]);
			chip_info->bin_map[list].BIN_addr =
			    byte_to_word(&fwdata[pos + 8]);
			if (chip_info->trim_id_table.support_hw_crc) {
				chip_info->bin_map[list].crc =
		...
		if (list >= (chip_info->ilm_dlm_num + info_sec_num)) {
			...
			pos = chip_info->bin_map[1].BIN_addr +
			      (0x10 *
			       (list - chip_info->ilm_dlm_num - info_sec_num));
			...
			snprintf(
			    chip_info->bin_map[list].name, 12, "Overlay-%d",
			    (list - chip_info->ilm_dlm_num - info_sec_num));
		}

		/* BIN size error detect */
		if ((chip_info->bin_map[list].BIN_addr +
		     chip_info->bin_map[list].size) > fwsize) {
			pr_info("access range (0x%08X to 0x%08X) is larger than bin size!\n",
				 chip_info->bin_map[list].BIN_addr,
				 chip_info->bin_map[list].BIN_addr +
				     chip_info->bin_map[list].size);
			return -EINVAL;
 ...
 ********************************************************/
/*
 *static int32_t Write_Partition(struct chip_data_nt36672c *chip_info, const u8
 ...
 *	    TPD_INFO("access range (0x%08X to 0x%08X) is larger than bin
 *size!\n",
			       ...
			       const u8 *fwdata, size_t fwsize)
{
	...
	for (list = 0; list < chip_info->partition; list++) {
		...
		//              TPD_INFO("[%d][%s] SRAM (0x%08X), SIZE (0x%08X),
		//              BIN (0x%08X)\n",
		//                              list, name, SRAM_addr, size,
		//                              BIN_addr);
		...
		for (i = 0; i < count; i++) {
			...
			memcpy(chip_info->fwbuf + 1, &fwdata[BIN_addr],
			       len); // payload
			CTP_SPI_WRITE(chip_info->s_client, chip_info->fwbuf,
				      len + 1);
...
static void nvt_set_bld_hw_crc(struct chip_data_nt36672c *chip_info)
{
	...
	nvt_set_bld_crc_bank(chip_info,
			     chip_info->trim_id_table.mmap->ILM_DES_ADDR,
			     chip_info->bin_map[0].SRAM_addr,
			     chip_info->trim_id_table.mmap->ILM_LENGTH_ADDR,
			     chip_info->bin_map[0].size,
			     chip_info->trim_id_table.mmap->G_ILM_CHECKSUM_ADDR,
			     chip_info->bin_map[0].crc);

	/* [1] DLM */
	/* write register bank */
	nvt_set_bld_crc_bank(chip_info,
			     chip_info->trim_id_table.mmap->DLM_DES_ADDR,
			     chip_info->bin_map[1].SRAM_addr,
			     chip_info->trim_id_table.mmap->DLM_LENGTH_ADDR,
			     chip_info->bin_map[1].size,
			     chip_info->trim_id_table.mmap->G_DLM_CHECKSUM_ADDR,
			     chip_info->bin_map[1].crc);
}
...
static void nvt_read_bld_hw_crc(struct chip_data_nt36672c *chip_info)
{
	...
	/* CRC Flag */
	nvt_set_page(chip_info,
		     chip_info->trim_id_table.mmap->BLD_ILM_DLM_CRC_ADDR);
	...
	nvt_set_page(chip_info,
		     chip_info->trim_id_table.mmap->R_ILM_CHECKSUM_ADDR);
	...
	nvt_set_page(chip_info,
		     chip_info->trim_id_table.mmap->R_DLM_CHECKSUM_ADDR);
...
static int32_t nvt_f2c_read_write(struct chip_data_nt36672c *chip_info,
				  uint8_t F2C_RW, uint32_t DDIC_REG_ADDR,
...
nvt_f2c_retry:
	...
	f2c_control = buf[2 + 1] | (0x01 << BIT_F2C_EN) |
	...
	// Step7: Check REG TH_CPU_CHK  status (1: Success,  0: Fail), if 0, can
	// Retry Step5.
	if (((buf[2] & 0x04) >> 2) != 0x01) {
		...
		if (f2c_retry <= retry_max) {
			...
			pr_info("check TH_CPU_CHK failed!, buf[1]=0x%02X, buf[2]=0x%02X\n",
				 buf[1], buf[2]);
			return -EIO;
		}
	}
	...
	if (F2C_RW == F2C_RW_READ) {
		...
		buf[1] = 0xFF;
		ret = CTP_SPI_READ(chip_info->s_client, buf,
				   1 + len); // 1 is AddrL
		if (ret) {
...
static int32_t nvt_f2c_disp_off(struct chip_data_nt36672c *chip_info)
{
	...
	return nvt_f2c_read_write(chip_info, F2C_RW_WRITE, DISP_OFF_ADDR, 1,
				  &data);
}
#endif /* NVT_TOUCH_ESD_DISP_RECOVERY */
					...
					const struct firmware *fw)
{
	...
	while (1) {
		...
		ret = nvt_check_fw_reset_state_noflash(chip_info,
						       RESET_STATE_INIT);
		if (ret) {
			TPD_INFO(
			    "nvt_check_fw_reset_state_noflash failed. (%d)\n",
			    ret);
			goto fail;
		} else {
			break;
...
fail:
		...
		if (unlikely(retry > 2) || chip_info->using_headfile) {
			...
			if (nvt_check_crc_done_ilm_err(chip_info)) {
				pr_info("set display off to trigger display esd recovery.\n");
...
/********* Start of implementation of custom_o_touchpanel_operations
 * callbacks*******************
 */
// extern int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info
// *panel_data);
...
static int nvt_power_control(void *chip_data, bool enable)
{
	...
	nvt_eng_reset(
	    chip_info); // make sure before nvt_bootloader_reset_noflash
...
static int nvt_get_usb_state(void) { return upmu_get_rgs_chrdet(); }
...
static u8 nvt_trigger_reason(void *chip_data, int gesture_enable,
			     int is_suspended)
{
...
static void nova_debug_info(struct chip_data_nt36672c *chip_info, u8 *buf)
{
	...
	// msleep(2000);
	if (tp_debug > 0) {
		pr_info("REK_INFO:0x%02X, RST_INFO:0x%02X\n",
		       chip_info->nvt_fw_debug_info.rek_info,
		       chip_info->nvt_fw_debug_info.rst_info);

		pr_info("ESD:0x%02X, PALM:0x%02X, BENDING:0x%02X, WATER:0x%02X, GND:0x%02X, ER:0x%02X\n",
		...
		pr_info(
		    "HOPPING:0x%02X, FOG:0x%02X, FILM:0x%02X, NOTCH:0x%02X\n\n",
		    chip_info->nvt_fw_debug_info.hopping,
...
static int32_t nvt_ts_point_data_checksum(uint8_t *buf, uint8_t length)
{
	...
	if (checksum != buf[length]) {
		pr_info("i2c/spi packet checksum not match. (point_data[%d]=0x%02X, checksum=0x%02X)\n",
				...
				int max_num)
{
	...
	// some kind of protect mechanism, after WDT firmware redownload and try
	// to save tp
	...
	for (i = 0; i < max_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) ||
			...
			chip_info->irq_timer =
			    jiffies; // reset esd check trigger base time
...
static void nvt_ts_wakeup_gesture_coordinate(uint8_t *data, uint8_t max_num)
{
	...
	for (i = 0; i < max_num; i++) {
		...
		if (((data[position] & 0x07) == 0x01) ||
		    ((data[position] & 0x07) == 0x02)) {
			input_x = (uint32_t)(data[position + 1] << 4) +
				  (uint32_t)(data[position + 3] >> 4);
			input_y = (uint32_t)(data[position + 2] << 4) +
				  (uint32_t)(data[position + 3] & 0x0F);
		}
		TPD_INFO("(%d: %d, %d)\n", i, input_x, input_y);
	}
}
...
static void
nvt_read_debug_gesture_coordinate_buffer(struct chip_data_nt36672c *chip_info,
					 uint32_t xdata_addr, u8 *xdata,
					 int32_t xdata_len)
{
	int32_t i = 0;
	int32_t j = 0;
	 ...
	 *    TPD_INFO("xdata read buffer(%d) less than max data size(%d),
	 *return\n", xdata_len, data_len);
	 *    return;
	 *}
	 */
	...
	for (i = 0; i < ((dummy_len + xdata_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / SPI_TANSFER_LENGTH); j++) {
			//---change xdata index---
			nvt_set_page(chip_info, head_addr +
						    (XDATA_SECTOR_SIZE * i) +
						    (SPI_TANSFER_LENGTH * j));

			//---read data---
			buf[0] = SPI_TANSFER_LENGTH * j;
			CTP_SPI_READ(chip_info->s_client, buf,
				     SPI_TANSFER_LENGTH + 1);
			...
			for (k = 0; k < SPI_TANSFER_LENGTH; k++) {
				...
				// (XDATA_SECTOR_SIZE*i + SPI_TANSFER_LENGTH*j +
				// k));
			}
		}
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / SPI_TANSFER_LENGTH + 1); j++) {
			...
			nvt_set_page(chip_info, xdata_addr + xdata_len -
						    residual_len +
						    (SPI_TANSFER_LENGTH * j));
	...
	//---remove dummy data---
	pr_info("nova read gesture xdata\n");
	for (i = 0; i < xdata_len; i++) {
		xdata[i] = xdata_tmp[dummy_len + i];
		pr_info("0x%02X,", xdata[i]);
	}
	pr_info("\n");
	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page(chip_info, chip_info->trim_id_table.mmap->EVENT_BUF_ADDR);
...
static void
nvt_dbg_gesture_record_coor_read(struct chip_data_nt36672c *chip_info,
				 u8 pointdata)
{
	u8 *xdata = NULL;
	uint32_t buf_len = 0;
	...
	nvt_read_debug_gesture_coordinate_buffer(
	    chip_info, NVT_MMAP_DEBUG_FINGER_DOWN_DIFFDATA, xdata, buf_len);
	...
	nvt_read_debug_gesture_coordinate_buffer(
	    chip_info, NVT_MMAP_DEBUG_STATUS_CHANGE_DIFFDATA, xdata, buf_len);
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	struct chip_data_nt36672c *chip_info =
	    (struct chip_data_nt36672c *)chip_data;
	uint8_t point_data[POINT_DATA_LEN + 2] = { 0 };
	uint8_t max_num = 10; // TODO: define max_num by custom_o common driver
	...
	ret = nvt_wdt_fw_recovery(chip_info, point_data);
	if (ret) {
		pr_info("receive all %02X, no gesture interrupts. recover for fw reset\n",
			 point_data[1]);
		nvt_reset(chip_info);
	...
	case RIGHT_SLIDE_DETECT:
		...
		gesture->Point_start.x =
		    (point_data[4] & 0xFF) | (point_data[5] & 0x0F) << 8;
		gesture->Point_start.y =
		    (point_data[6] & 0xFF) | (point_data[7] & 0x0F) << 8;
		gesture->Point_end.x =
		    (point_data[8] & 0xFF) | (point_data[9] & 0x0F) << 8;
		gesture->Point_end.y =
		    (point_data[10] & 0xFF) | (point_data[11] & 0x0F) << 8;
		break;
	...
	case UP_VEE_DETECT:
		...
		gesture->Point_start.y =
		    (point_data[6] & 0xFF) | (point_data[7] & 0x0F) << 8;
		gesture->Point_end.x =
		    (point_data[12] & 0xFF) | (point_data[13] & 0x0F) << 8;
		gesture->Point_end.y =
		    (point_data[14] & 0xFF) | (point_data[15] & 0x0F) << 8;
		gesture->Point_1st.x =
		    (point_data[8] & 0xFF) | (point_data[9] & 0x0F) << 8;
		gesture->Point_1st.y =
		    (point_data[10] & 0xFF) | (point_data[11] & 0x0F) << 8;
	...
	case CIRCLE_DETECT:
		...
		gesture->Point_start.y =
		    (point_data[6] & 0xFF) | (point_data[7] & 0x0F) << 8;
		gesture->Point_1st.x = (point_data[8] & 0xFF) |
				       (point_data[9] & 0x0F) << 8; // ymin
		gesture->Point_1st.y =
		    (point_data[10] & 0xFF) | (point_data[11] & 0x0F) << 8;
		gesture->Point_2nd.x = (point_data[12] & 0xFF) |
				       (point_data[13] & 0x0F) << 8; // xmin
		gesture->Point_2nd.y =
		    (point_data[14] & 0xFF) | (point_data[15] & 0x0F) << 8;
		gesture->Point_3rd.x = (point_data[16] & 0xFF) |
				       (point_data[17] & 0x0F) << 8; // ymax
		gesture->Point_3rd.y =
		    (point_data[18] & 0xFF) | (point_data[19] & 0x0F) << 8;
		gesture->Point_4th.x = (point_data[20] & 0xFF) |
				       (point_data[21] & 0x0F) << 8; // xmax
		gesture->Point_4th.y =
		    (point_data[22] & 0xFF) | (point_data[23] & 0x0F) << 8;
		gesture->Point_end.x =
		    (point_data[24] & 0xFF) | (point_data[25] & 0x0F) << 8;
		gesture->Point_end.y =
		    (point_data[26] & 0xFF) | (point_data[27] & 0x0F) << 8;
		break;
	...
	case DOUSWIP_DETECT:
		...
		gesture->Point_1st.y =
		    (point_data[10] & 0xFF) | (point_data[11] & 0x0F) << 8;
		gesture->Point_2nd.x =
		    (point_data[16] & 0xFF) | (point_data[17] & 0x0F) << 8;
		gesture->Point_2nd.y =
		    (point_data[18] & 0xFF) | (point_data[19] & 0x0F) << 8;
		break;
	...
	case M_DETECT:
		...
		gesture->Point_3rd.y =
		    (point_data[18] & 0xFF) | (point_data[19] & 0x0F) << 8;
		gesture->Point_end.x =
		    (point_data[20] & 0xFF) | (point_data[21] & 0x0F) << 8;
		gesture->Point_end.y =
		    (point_data[22] & 0xFF) | (point_data[23] & 0x0F) << 8;
		break;
	...
	pr_info("%s, gesture_id: 0x%x, func_type: 0x%x, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
	    __func__, gesture_id, func_type, gesture->gesture_type,
...
#ifdef CONFIG_CUSTOM_O_TP_APK
static __maybe_unused int nvt_enable_debug_gesture_coordinate_record_mode(
	struct chip_data_nt36672c *chip_info, bool enable)
{
	...
	if (enable) {
		ret = nvt_extend_cmd_store(
		    chip_info, EVENTBUFFER_EXT_CMD,
...
nvt_enable_debug_gesture_coordinate_mode(struct chip_data_nt36672c *chip_info,
					 bool enable)
{
	int8_t ret = -1;
...
#ifdef CONFIG_CUSTOM_O_TP_APK
		if (chip_info->debug_gesture_sta) {
			nvt_enable_debug_gesture_coordinate_record_mode(
			    chip_info, true);
		}
#endif // end of CONFIG_CUSTOM_O_TP_APK
				 ...
				 int state)
{
	...
	pr_info("%s:state = %d, limit_corner = %d, level = %d, chip_info->is_sleep_writed = %d\n",
		 __func__, state, ts->limit_corner, edge_limit_level,
		 chip_info->is_sleep_writed);

	if (state == 1 || chip_info->touch_direction == VERTICAL_SCREEN) {
		ret = nvt_extend_cmd_store(chip_info,
					   EVENTBUFFER_EDGE_LIMIT_VERTICAL,
					   edge_limit_level);
	} else {
		if (chip_info->touch_direction == LANDSCAPE_SCREEN_90) {
			ret = nvt_extend_cmd_store(
			    chip_info, EVENTBUFFER_EDGE_LIMIT_RIGHT_UP,
			    edge_limit_level);
		} else if (chip_info->touch_direction == LANDSCAPE_SCREEN_270) {
			ret = nvt_extend_cmd_store(
			    chip_info, EVENTBUFFER_EDGE_LIMIT_LEFT_UP,
			    edge_limit_level);
		}
	}
...
#ifdef CONFIG_CUSTOM_O_TP_APK
static __maybe_unused int
nvt_enable_hopping_polling_mode(struct chip_data_nt36672c *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_hopping_fix_freq_mode(struct chip_data_nt36672c *chip_info,
				 bool enable)
{
	int8_t ret = -1;
	...
	if (enable)
		...
		ret =
		    nvt_cmd_store(chip_info, EVENTBUFFER_HOPPING_FIX_FREQ_OFF);

	return ret;
}

static __maybe_unused int
nvt_enable_debug_msg_diff_mode(struct chip_data_nt36672c *chip_info,
			       bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_water_polling_mode(struct chip_data_nt36672c *chip_info, bool enable)
{
	int8_t ret = -1;
	...
	if (enable) {
		ret =
		    nvt_extend_cmd_store(chip_info, EVENTBUFFER_EXT_CMD,
...
static int nvt_mode_switch(void *chip_data, enum work_mode mode, bool flag)
{
	int ret = -1;
	struct chip_data_nt36672c *chip_info =
	    (struct chip_data_nt36672c *)chip_data;
	...
	/* Debug Function */
	/*case MODE_HOPPING_POLLING:	//TODO: add define in custom_o h file,
	 ...
	 *    break;
	 *
	 *case MODE_HOPPING_FIX_FREQ:	//TODO: add define in custom_o h file,
	...
	/*case DEBUG_MODE_MESSAGE_DIFF:	//TODO: add define in custom_o h file,
	 *DEBUG_MODE_MESSAGE_DIFF
	 ...
	 *    break;
	 *
	 *case DEBUG_MODE_GESTURE_COORDINATE:	//TODO: add define in custom_o h
	 *file, DEBUG_MODE_GESTURE_COORDINATE
	 ...
	 *    break;
	 *
	 *case DEBUG_MODE_GESTURE_COORDINATE_RECORD:	//TODO: add define in
	 *custom_o h file, DEBUG_MODE_GESTURE_COORDINATE_RECORD
	 *    ret = nvt_enable_debug_gesture_coordinate_record_mode(chip_info,
	 *flag);
	 *    if (ret < 0) {
	 *	    TPD_INFO("%s: enable debug gesture coordinate record %d
	 *failed\n", __func__, flag);
	 *    }
	 *    break;
	 */
...
static enum fw_check_state nvt_fw_check(void *chip_data,
				   ...
				   struct panel_info *panel_data)
{
	...
	snprintf(dev_version, MAX_DEVICE_VERSION_LENGTH, "%02X",
		 panel_data->TP_FW);
	if (panel_data->manufacture_info.version) {
		strlcpy(tmp_version,
			panel_data->manufacture_info.version,
			panel_data->version_len + 1);
		if (panel_data->vid_len == 0) {
		//ver_len =
		//strlen(panel_data->manufacture_info.version);
		//if (ver_len <= 8) {
		//  strlcat(panel_data->manufacture_info.version,
		//  dev_version,
		//  MAX_DEVICE_VERSION_LENGTH);
		//} else {
		//    strlcpy(&panel_data->manufacture_info.version[8],
		//    dev_version,
		//    MAX_DEVICE_VERSION_LENGTH -
		//    8);
		//}
			strlcat(tmp_version, dev_version,
				panel_data->version_len + 3);
			strlcpy(panel_data->manufacture_info.version,
				tmp_version,
				panel_data->version_len + 3);
		} else {
			...
			strlcpy(&panel_data->manufacture_info
				     .version[ver_len],
				dev_version,
...
static void nvt_read_mdata(struct chip_data_nt36672c *chip_info,
			   uint32_t xdata_addr, int32_t *xdata,
			   int32_t xdata_len)
{
	int32_t i = 0;
	int32_t j = 0;
	...
	if (xdata_len / sizeof(int32_t) < data_len / 2) {
		pr_info("xdata read buffer(%d) less than max data size(%d), return\n",
			 xdata_len, data_len);
		return;
	}
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / SPI_TANSFER_LENGTH + 1); j++) {
			//---change xdata index---
			nvt_set_page(chip_info, xdata_addr + data_len -
						    residual_len +
						    (SPI_TANSFER_LENGTH * j));
...
static int32_t
nvt_polling_hand_shake_status(struct chip_data_nt36672c *chip_info)
{
	uint8_t buf[8] = { 0 };
	int32_t i = 0;
...
static int32_t nvt_read_fw_noise(struct chip_data_nt36672c *chip_info,
				 int32_t config_Diff_Test_Frame, int32_t *xdata,
				 int32_t *xdata_n, int32_t xdata_len)
{
	...
	if (nvt_get_fw_pipe(chip_info) == 0) {
		nvt_read_mdata(chip_info,
			       chip_info->trim_id_table.mmap->DIFF_PIPE0_ADDR,
			       xdata, xdata_len);
	} else {
		nvt_read_mdata(chip_info,
			       chip_info->trim_id_table.mmap->DIFF_PIPE1_ADDR,
			       xdata, xdata_len);
	}
...
static int32_t nvt_check_bin_checksum(const u8 *fwdata, size_t fwsize)
{
	...
	checksum_calculated =
	    CheckSum(fwdata, fwsize - FW_BIN_CHECKSUM_LEN - 1);
...
static int32_t nvt_get_fw_need_write_size(const struct firmware *fw)
{
	...
	for (i = total_sectors_to_check; i > 0; i--) {
		...
		if (strncmp(&fw->data[i * FLASH_SECTOR_SIZE -
			...
			fw_need_write_size = i * FLASH_SECTOR_SIZE;
			TPD_INFO(
			    "fw_need_write_size = %zu(0x%zx), MOD end flag\n",
			    fw_need_write_size, fw_need_write_size);
			return 0;
		}
	}
					 ...
					 const struct firmware *fw, bool force)
{
	...
	// TPD_INFO("bin file size not match (%zu), change to use headfile
	// fw.\n", fw->size);
	// goto out_fail;
	// release_firmware(fw);
	// fw = NULL;
	...
	if (fw == NULL) {
		...
		if (chip_info->g_fw_sta) {
			TPD_INFO(
			    "request firmware failed, get from g_fw_buf\n");
			...
			if (chip_info->p_firmware_headfile->firmware_data) {
				request_fw_headfile->size =
				    chip_info->p_firmware_headfile
					->firmware_size;
				request_fw_headfile->data =
				    chip_info->p_firmware_headfile
					->firmware_data;
				fw = request_fw_headfile;

			} else {
				pr_info("firmware_data is NULL! exit firmware update!\n");
				goto out_fail;
			}
		}
	}
	...
	if (ret) {
		if (fw != request_fw_headfile) {
			pr_info("Image fw check checksum failed, reload fw from array\n");

			goto out_fail;

		} else {
			TPD_INFO(
			    "array fw check checksum failed, but use still\n");
		}
	} else {
...
static void nvt_store_testrecord_to_file(int fd, char *item, uint8_t result,
					 uint8_t *record, int txnum, int rxnum)
{
	uint8_t i, j;
	int32_t iArrayIndex = 0;
	...
	} else if (result == NVT_MP_UNKNOWN) {
		store_to_file(fd, "%s unknown!\n", item);
...
static void nvt_store_testdata_to_file(int fd, char *item, uint8_t result,
				       int32_t *rawdata, int txnum, int rxnum)
{
	uint8_t i, j;
	int32_t iArrayIndex = 0;
	...
	if (result == NVT_MP_FAIL_READ_DATA) {
		...
		store_to_file(fd, "read %s unknown!\n", item);
		goto exit;
	}
...
static int32_t nvt_read_doze_fw_noise(struct chip_data_nt36672c *chip_info,
				      int32_t config_Doze_Noise_Test_Frame,
				      int32_t doze_X_Channel, int32_t *xdata,
				      int32_t *xdata_n, int32_t xdata_len)
{
	uint8_t buf[128] = { 0 };
	uint32_t x = 0;
	...
	for (x = 0; x < doze_X_Channel; x++) {
		...
		//---read data---
		buf[0] = (chip_info->trim_id_table.mmap->DIFF_PIPE0_ADDR +
			  rx_num * doze_X_Channel * x) &
			 0xFF;
		CTP_SPI_READ(chip_info->s_client, buf, rx_num * 2 + 1);

		for (y = 0; y < rx_num; y++) {
			xdata[y * doze_X_Channel + x] =
			    (uint16_t)(buf[y * doze_X_Channel + 1] +
				       256 * buf[y * doze_X_Channel + 2]);
		}
	}
	...
	for (y = 0; y < rx_num; y++) {
		for (x = 0; x < doze_X_Channel; x++) {
			...
			xdata_n[iArrayIndex] =
			    (int8_t)(xdata[iArrayIndex] & 0xFF) * 4;
			xdata[iArrayIndex] =
			    (int8_t)((xdata[iArrayIndex] >> 8) & 0xFF) *
			    4; // scaling up
		}
	...
	//---Leave Test Mode---
	// nvt_change_mode(NORMAL_MODE);    //No return to normal mode.
	// Continuous to get doze rawdata
				      ...
				      int32_t xdata_len)
{
	...
	if (xdata_len / sizeof(int32_t) < rm_num * doze_X_Channel) {
		TPD_INFO(
		    "read doze baseline buffer(%d) less than data size(%d)\n",
	...
	for (x = 0; x < doze_X_Channel; x++) {
		...
		nvt_set_page(
		    chip_info,
		    chip_info->trim_id_table.mmap->DOZE_GM_S1D_SCAN_RAW_ADDR +
		...
		for (y = 0; y < rm_num; y++) {
			xdata[y * 2 + x] =
			    (uint16_t)(buf[y * 2 + 1] + 256 * buf[y * 2 + 2]);
		}
	}
...
static void nvt_black_screen_test(void *chip_data, char *message)
{
	...
	/* test result */
	uint8_t rawdata_result = NVT_MP_UNKNOWN;
	uint8_t noise_p_result = NVT_MP_UNKNOWN;
	uint8_t noise_n_result = NVT_MP_UNKNOWN;
	uint8_t fdm_noise_p_result = NVT_MP_UNKNOWN;
	uint8_t fdm_noise_n_result = NVT_MP_UNKNOWN;
	uint8_t fdm_rawdata_result = NVT_MP_UNKNOWN;
	/* record data */
	...
	TPD_INFO("%s : update test firmware success\n", __func__);
	...
	if (nvt_check_fw_reset_state_noflash(chip_info,
	...
	if (!(raw_data && noise_p_data && noise_n_data && fdm_raw_data &&
	      fdm_noise_p_data && fdm_noise_n_data)) {
		TPD_INFO("kzalloc space failed\n");
		snprintf(message, MESSAGE_SIZE,
			 "1 error, kzalloc space failed\n");
		goto RELEASE_DATA;
	}
	...
	ret = request_firmware(&fw, chip_info->test_limit_name,
	...
	lpwg_rawdata_P =
	    (int32_t *)(fw->data + ph->array_LPWG_Rawdata_P_offset);
	lpwg_rawdata_N =
	    (int32_t *)(fw->data + ph->array_LPWG_Rawdata_N_offset);
	lpwg_diff_rawdata_P =
	    (int32_t *)(fw->data + ph->array_LPWG_Diff_P_offset);
	lpwg_diff_rawdata_N =
	    (int32_t *)(fw->data + ph->array_LPWG_Diff_N_offset);
	fdm_rawdata_P = (int32_t *)(fw->data + ph->array_FDM_Rawdata_P_offset);
	fdm_rawdata_N = (int32_t *)(fw->data + ph->array_FDM_Rawdata_N_offset);
	fdm_diff_rawdata_P =
	    (int32_t *)(fw->data + ph->array_FDM_Diff_P_offset);
	fdm_diff_rawdata_N =
	    (int32_t *)(fw->data + ph->array_FDM_Diff_N_offset);
	//---FW Rawdata Test---
	...
	if (nvt_get_fw_pipe(chip_info) == 0)
		nvt_read_mdata(chip_info,
			       chip_info->trim_id_table.mmap->RAW_PIPE0_ADDR,
			       raw_data, buf_len);
	else
		nvt_read_mdata(chip_info,
			       chip_info->trim_id_table.mmap->RAW_PIPE1_ADDR,
			       raw_data, buf_len);
	if ((ph->config_Lmt_LPWG_Rawdata_P != 0) &&
	    (ph->config_Lmt_LPWG_Rawdata_N != 0)) {
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((raw_data[iArrayIndex] >
					...
					pr_info("LPWG_Rawdata Test failed at rawdata[%d][%d] = %d[%d %d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((raw_data[iArrayIndex] >
					...
					rawdata_result = NVT_MP_FAIL;
					raw_record[iArrayIndex] = 1;
					pr_info("LPWG_Rawdata Test failed at rawdata[%d][%d] = %d\n",
						 i, j, raw_data[iArrayIndex]);
					if (!err_cnt) {
						snprintf(buf, 128,
				"LPWG Rawdata[%d][%d] = %d[%d %d]\n",
	...
	if (nvt_read_fw_noise(chip_info, ph->config_Diff_Test_Frame,
			      noise_p_data, noise_n_data, buf_len) != 0) {
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((noise_p_data[iArrayIndex] >
				     ph->config_Lmt_LPWG_Diff_P) ||
				    (noise_p_data[iArrayIndex] <
				     ph->config_Lmt_LPWG_Diff_N)) {
					noise_p_result = NVT_MP_FAIL;
					noise_p_record[iArrayIndex] = 1;
					pr_info("LPWG Noise RawData_Diff_Max Test failed at rawdata[%d][%d] = %d[%d %d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((noise_n_data[iArrayIndex] >
					...
					pr_info("LPWG Noise RawData_Diff_Min Test failed at rawdata[%d][%d] = %d[%d %d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				TPD_DEBUG_NTAG("%d, ",
					       noise_p_data[iArrayIndex]);
				if ((noise_p_data[iArrayIndex] >
				     lpwg_diff_rawdata_P[iArrayIndex]) ||
				    (noise_p_data[iArrayIndex] <
				     lpwg_diff_rawdata_N[iArrayIndex])) {
					noise_p_result = NVT_MP_FAIL;
					noise_p_record[iArrayIndex] = 1;
					pr_info("LPWG Noise RawData_Diff_Max Test failed at rawdata[%d][%d] = %d\n",
						 i, j, raw_data[iArrayIndex]);
					if (!err_cnt) {
						snprintf(buf, 128,
		"LPWG Noise RawData_Diff_Max[%d][%d] = %d[%d %d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((noise_n_data[iArrayIndex] >
					...
					noise_n_result = NVT_MP_FAIL;
					noise_n_record[iArrayIndex] = 1;
					pr_info("LPWG Noise RawData_Diff_Min Test failed at rawdata[%d][%d] = %d\n",
					    i, j, noise_n_data[iArrayIndex]);
					if (!err_cnt) {
						snprintf(buf, 128,
	"LPWG Noise RawData_Diff_Min[%d][%d] = %d[%d %d]\n",
	...
	if (nvt_read_doze_fw_noise(chip_info, ph->config_FDM_Noise_Test_Frame,
				   ph->fdm_X_Channel, fdm_noise_p_data,
				   fdm_noise_n_data, buf_len) != 0) {
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->fdm_X_Channel; i++) {
				...
				if ((fdm_noise_n_data[iArrayIndex] >
					...
					pr_info("FDM Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j,
						 fdm_noise_n_data[iArrayIndex],
						 ph->config_Lmt_FDM_Diff_N,
						 ph->config_Lmt_FDM_Diff_P);
...
"FDM Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->fdm_X_Channel; i++) {
				iArrayIndex = j * ph->fdm_X_Channel + i;
				if ((fdm_noise_n_data[iArrayIndex] >
				     fdm_diff_rawdata_P[iArrayIndex]) ||
				    (fdm_noise_n_data[iArrayIndex] <
				     fdm_diff_rawdata_N[iArrayIndex])) {
					fdm_noise_p_result = NVT_MP_FAIL;
					fdm_noise_p_record[iArrayIndex] = 1;
					pr_info("FDM Noise RawData_Diff_Max Test failed at data[%d][%d] = %d [%d,%d]\n",
...
"FDM Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
	...
	if (nvt_read_doze_baseline(chip_info, ph->fdm_X_Channel, fdm_raw_data,
				   buf_len) != 0) {
		TPD_INFO("read FDM FW Rawdata failed!\n");
		fdm_rawdata_result = NVT_MP_FAIL_READ_DATA;
		snprintf(buf, 128, "read FDM FW Rawdata failed!\n");
		err_cnt++;
		goto TEST_END;
	}

	if ((ph->config_Lmt_FDM_Rawdata_P != 0) &&
	    (ph->config_Lmt_FDM_Rawdata_N != 0)) {
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->fdm_X_Channel; i++) {
				iArrayIndex = j * ph->fdm_X_Channel + i;
				if ((fdm_raw_data[iArrayIndex] >
				     ph->config_Lmt_FDM_Rawdata_P) ||
				    (fdm_raw_data[iArrayIndex] <
				     ph->config_Lmt_FDM_Rawdata_N)) {
					fdm_rawdata_result = NVT_MP_FAIL;
					fdm_raw_record[iArrayIndex] = 1;
					pr_info("FDM FW Rawdata Test failed at data[%d][%d] = %d [%d,%d]\n",
					    i, j, fdm_raw_data[iArrayIndex],
					    ph->config_Lmt_FDM_Rawdata_N,
					    ph->config_Lmt_FDM_Rawdata_P);
					if (!err_cnt) {
						snprintf(buf, 128,
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->fdm_X_Channel; i++) {
				iArrayIndex = j * ph->fdm_X_Channel + i;
				if ((fdm_raw_data[iArrayIndex] >
				     fdm_rawdata_P[iArrayIndex]) ||
				    (fdm_raw_data[iArrayIndex] <
				     fdm_rawdata_N[iArrayIndex])) {
					fdm_rawdata_result = NVT_MP_FAIL;
					fdm_raw_record[iArrayIndex] = 1;
					pr_info("FDM FW Rawdata Test failed at data[%d][%d] = %d [%d,%d]\n",
					    i, j, fdm_raw_data[iArrayIndex],
...
TEST_END:
	...
	char *prex = "/sdcard/TpTestReport/screenOff";

	if (!err_cnt) {
		snprintf(all_test_result, 8, "PASS");
		snprintf(data_buf, 128,
		"%s/OK/tp_testlimit_%s_%02d%02d%02d-%02d%02d%02d-utc.csv",
			 prex, all_test_result,
		...
		snprintf(data_buf, 128,
		"%s/NG/tp_testlimit_%s_%02d%02d%02d-%02d%02d%02d-utc.csv",
			 prex, all_test_result,
			 (rtc_now_time.tm_year + 1900) % 100,
			 rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			 rtc_now_time.tm_hour, rtc_now_time.tm_min,
	...
	/* Test Item */
	/* 1. rawdata        */
	nvt_store_testrecord_to_file(fd, "LPWG mode FW Rawdata", rawdata_result,
				     raw_record, tx_num, rx_num);
	/* 2. noise max      */
	nvt_store_testrecord_to_file(fd, "LPWG Noise RawData_Diff_Max",
				     noise_p_result, noise_p_record, tx_num,
				     rx_num);
	/* 3. noise min      */
	nvt_store_testrecord_to_file(fd, "LPWG Noise RawData_Diff_Min",
				     noise_n_result, noise_n_record, tx_num,
				     rx_num);
	/* 4. fdm noise max */
	nvt_store_testrecord_to_file(fd, "FDM RawData_Diff_Max",
				     fdm_noise_p_result, fdm_noise_p_record,
				     ph->fdm_X_Channel, rx_num);
	/* 5. fdm noise min */
	nvt_store_testrecord_to_file(fd, "FDM RawData_Diff_Min",
				     fdm_noise_n_result, fdm_noise_n_record,
				     ph->fdm_X_Channel, rx_num);
	/* 6. fdm rawdata   */
	nvt_store_testrecord_to_file(fd, "FDM FW Rawdata", fdm_rawdata_result,
				     fdm_raw_record, ph->fdm_X_Channel, rx_num);

	/* 1. rawdata        */
	nvt_store_testdata_to_file(fd, "LPWG mode FW Rawdata", rawdata_result,
				   raw_data, tx_num, rx_num);
	/* 2. noise max      */
	nvt_store_testdata_to_file(fd, "LPWG Noise RawData_Diff_Max",
				   noise_p_result, noise_p_data, tx_num,
				   rx_num);
	/* 3. noise min      */
	nvt_store_testdata_to_file(fd, "LPWG Noise RawData_Diff_Min",
				   noise_n_result, noise_n_data, tx_num,
				   rx_num);
	/* 4. fdm noise max */
	nvt_store_testdata_to_file(fd, "FDM RawData_Diff_Max",
				   fdm_noise_p_result, fdm_noise_p_data,
				   ph->fdm_X_Channel, rx_num);
	/* 5. fdm noise min */
	nvt_store_testdata_to_file(fd, "FDM RawData_Diff_Min",
				   fdm_noise_n_result, fdm_noise_n_data,
				   ph->fdm_X_Channel, rx_num);
	/* 6. fdm rawdata   */
	nvt_store_testdata_to_file(fd, "FDM FW Rawdata", fdm_rawdata_result,
				   fdm_raw_data, ph->fdm_X_Channel, rx_num);
...
static struct custom_o_touchpanel_operations nvt_ops = {
...
static void nvt_data_read(struct seq_file *s,
			  struct chip_data_nt36672c *chip_info,
			  enum DEBUG_READ_TYPE read_type)
{
	int ret = -1;
	int i, j;
	uint8_t pipe;
	...
	case NVT_DIFFDATA:
		...
		break;
	case NVT_BASEDATA:
		seq_puts(s, "basline_data:\n");
		nvt_read_mdata(chip_info,
			       chip_info->trim_id_table.mmap->BASELINE_ADDR,
			       xdata, buf_len);
		break;
	default:
		seq_puts(s, "read type not support\n");
		break;
	}
	...
	for (i = 0; i < chip_info->hw_res->RX_NUM; i++) {
		...
		for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
			seq_printf(s, "%5d, ",
				   xdata[i * chip_info->hw_res->TX_NUM + j]);
		}
		seq_puts(s, "\n");
	}
				...
				DEBUG_READ_TYPE read_type)
{
	...
	case NVT_DEBUG_FINGER_DOWN_DIFFDATA:
		seq_puts(s, "debug finger down diff data:\n");
		nvt_read_debug_mdata(chip_info,
				     NVT_MMAP_DEBUG_FINGER_DOWN_DIFFDATA, xdata,
				     buf_len);
		break;

	case NVT_DEBUG_STATUS_CHANGE_DIFFDATA:
		seq_puts(s, "debug status change diff data:\n");
		nvt_read_debug_mdata(chip_info,
...
#ifdef CONFIG_CUSTOM_O_TP_APK
	if (chip_info->debug_mode_sta) {
		nvt_debug_data_read(s, chip_info,
				    NVT_DEBUG_FINGER_DOWN_DIFFDATA);
	}
#endif // end of CONFIG_CUSTOM_O_TP_APK
}
...
#ifdef CONFIG_CUSTOM_O_TP_APK
static __maybe_unused void nvt_dbg_diff_finger_down_read(struct seq_file *s,
							 void *chip_data)
{
	struct chip_data_nt36672c *chip_info =
	    (struct chip_data_nt36672c *)chip_data;
...
static __maybe_unused void nvt_dbg_diff_status_change_read(struct seq_file *s,
							   void *chip_data)
{
	struct chip_data_nt36672c *chip_info =
	    (struct chip_data_nt36672c *)chip_data;
...
static struct debug_info_proc_operations debug_info_proc_ops = {
	...
	//.dbg_diff_finger_down_read = nvt_dbg_diff_finger_down_read, //TODO:
	//link to custom_o common driver
	//.dbg_diff_status_change_read = nvt_dbg_diff_status_change_read,
	////TODO: link to custom_o common driver
	//.dbg_gesture_record_coor_read = nvt_dbg_gesture_record_coor_read,
	////TODO: link to custom_o common driver
				 ...
				 int32_t *xdata, int32_t xdata_len)
{
	...
	rawdata_buf = kzalloc(chip_info->hw_res->TX_NUM *
					     chip_info->hw_res->RX_NUM * 2,
					 GFP_KERNEL);
	if (!rawdata_buf) {
	...
	for (y = 0; y < chip_info->hw_res->RX_NUM; y++) {
		//---change xdata index---
		nvt_set_page(chip_info,
			     raw_pipe_addr + y * chip_info->hw_res->TX_NUM * 2);
		buf[0] = (uint8_t)(
		    (raw_pipe_addr + y * chip_info->hw_res->TX_NUM * 2) & 0xFF);
		CTP_SPI_READ(chip_info->s_client, buf,
			     chip_info->hw_res->TX_NUM * 2 + 1);
				...
				int32_t *xdata, int32_t xdata_len)
{
	...
	rawdata_buf = kzalloc(tx_num * rx_num * 2, GFP_KERNEL);
	if (!rawdata_buf) {
		TPD_INFO("kzalloc for rawdata_buf failed!\n");
		return -ENOMEM;
	}
	...
	for (y = 0; y < rx_num; y++) {
		for (x = 0; x < tx_num; x++) {
			// xdata[(rx_num-y-1) * tx_num + (tx_num-x-1)] =
			// (int16_t)((rawdata_buf[(y*tx_num + x)*2] + 256 *
			// rawdata_buf[(y*tx_num + x)*2 + 1]));
			xdata[y * tx_num + x] = (int16_t)(
			    (rawdata_buf[(y * tx_num + x) * 2] +
			     256 * rawdata_buf[(y * tx_num + x) * 2 + 1]));
		}
	}
...
static void nvt_auto_test(struct seq_file *s, void *chip_data,
			  struct nvt_testdata *nvt_testdata)
{
	...
	/* test result */
	uint8_t rawdata_result = NVT_MP_UNKNOWN;
	uint8_t cc_result = NVT_MP_UNKNOWN;
	uint8_t noise_p_result = NVT_MP_UNKNOWN;
	uint8_t noise_n_result = NVT_MP_UNKNOWN;
	uint8_t doze_noise_p_result = NVT_MP_UNKNOWN;
	uint8_t doze_noise_n_result = NVT_MP_UNKNOWN;
	uint8_t doze_rawdata_result = NVT_MP_UNKNOWN;
	uint8_t short_result = NVT_MP_UNKNOWN;
	uint8_t open_result = NVT_MP_UNKNOWN;
	/* record data */
	uint8_t *raw_record = NULL;
	...
	TPD_INFO("update test firmware success!\n");

	if (nvt_switch_FreqHopEnDis(chip_info, FREQ_HOP_DISABLE)) {
		TPD_INFO("switch frequency hopping disable failed!\n");
		seq_puts(s, "switch frequency hopping disable failed!\n");
		goto RELEASE_FIRMWARE;
	}
	...
	if (nvt_check_fw_reset_state_noflash(chip_info,
					     RESET_STATE_NORMAL_RUN)) {
		TPD_INFO("check fw reset state failed!\n");
		seq_puts(s, "check fw reset state failed!\n");
		goto RELEASE_FIRMWARE;
	}
	...
	if (nvt_clear_fw_status(chip_info)) {
		TPD_INFO("clear fw status failed!\n");
		seq_puts(s, "clear fw status failed!\n");
		goto RELEASE_FIRMWARE;
	}
	...
	if (nvt_check_fw_status(chip_info)) {
		TPD_INFO("check fw status failed!\n");
		seq_puts(s, "check fw status failed!\n");
		goto RELEASE_FIRMWARE;
	}

	if (nvt_get_fw_info_noflash(chip_info)) {
		TPD_INFO("get fw info failed!\n");
		seq_puts(s, "get fw info failed!\n");
		goto RELEASE_FIRMWARE;
	}
	...
	if (!(raw_data && cc_data && noise_p_data && noise_n_data &&
		...
		err_cnt++;
		seq_puts(s, "malloc memory failed!\n");
	...
	fw_rawdata_P =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_fw_rawdata_P_offset);
	fw_rawdata_N =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_fw_rawdata_N_offset);
	open_rawdata_P = (int32_t *)(nvt_testdata->fw->data +
				     ph->array_open_rawdata_P_offset);
	open_rawdata_N = (int32_t *)(nvt_testdata->fw->data +
				     ph->array_open_rawdata_N_offset);
	short_rawdata_P = (int32_t *)(nvt_testdata->fw->data +
				      ph->array_Short_Rawdata_P_offset);
	short_rawdata_N = (int32_t *)(nvt_testdata->fw->data +
				      ph->array_Short_Rawdata_N_offset);
	diff_rawdata_P =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_FW_Diff_P_offset);
	diff_rawdata_N =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_FW_Diff_N_offset);
	cc_data_P =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_FW_CC_P_offset);
	cc_data_N =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_FW_CC_N_offset);
	doze_rawdata_P = (int32_t *)(nvt_testdata->fw->data +
				     ph->array_Doze_Rawdata_P_offset);
	doze_rawdata_N = (int32_t *)(nvt_testdata->fw->data +
				     ph->array_Doze_Rawdata_N_offset);
	doze_diff_rawdata_P =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_Doze_Diff_P_offset);
	doze_diff_rawdata_N =
	    (int32_t *)(nvt_testdata->fw->data + ph->array_Doze_Diff_N_offset);
	...
	for (j = 0; j < rx_num; j++) {
		for (i = 0; i < tx_num; i++) {
			iArrayIndex = j * tx_num + i;
			if ((raw_data[iArrayIndex] >
			     fw_rawdata_P[iArrayIndex]) ||
			    (raw_data[iArrayIndex] <
			     fw_rawdata_N[iArrayIndex])) {
				rawdata_result = NVT_MP_FAIL;
				raw_record[iArrayIndex] = 1;
				pr_info("rawdata Test failed at rawdata[%d][%d] = %d [%d,%d]\n",
					 i, j, raw_data[iArrayIndex],
					 fw_rawdata_N[iArrayIndex],
					 fw_rawdata_P[iArrayIndex]);
				if (!err_cnt) {
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((cc_data[iArrayIndex] >
				     ph->config_Lmt_FW_CC_P) ||
				    (cc_data[iArrayIndex] <
				     ph->config_Lmt_FW_CC_N)) {
					cc_result = NVT_MP_FAIL;
					cc_record[iArrayIndex] = 1;
					pr_info("cc data Test failed at rawdata[%d][%d] = %d [%d,%d]\n",
						 i, j, cc_data[iArrayIndex],
						 ph->config_Lmt_FW_CC_N,
						 ph->config_Lmt_FW_CC_P);
					if (!err_cnt) {
						seq_printf(s,
"cc data Test failed at rawdata[%d][%d] = %d [%d,%d]\n",
						    i, j, cc_data[iArrayIndex],
						    ph->config_Lmt_FW_CC_N,
						    ph->config_Lmt_FW_CC_P);
					}
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				iArrayIndex = j * tx_num + i;
				if ((cc_data[iArrayIndex] >
				     cc_data_P[iArrayIndex]) ||
				    (cc_data[iArrayIndex] <
				     cc_data_N[iArrayIndex])) {
					cc_result = NVT_MP_FAIL;
					cc_record[iArrayIndex] = 1;
					pr_info("cc data Test failed at rawdata[%d][%d] = %d [%d,%d]\n",
						 i, j, cc_data[iArrayIndex],
...
"cc data Test failed at rawdata[%d][%d] = %d [%d,%d]\n",
	...
	if (nvt_read_fw_noise(chip_info, ph->config_Diff_Test_Frame,
		...
		noise_p_result = NVT_MP_FAIL_READ_DATA;
		noise_n_result = NVT_MP_FAIL_READ_DATA;
		seq_puts(s, "read Noise data failed!\n");
		err_cnt++;
		goto TEST_END;
	}

	if ((ph->config_Lmt_FW_Diff_P != 0) &&
	    (ph->config_Lmt_FW_Diff_N != 0)) {
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				iArrayIndex = j * tx_num + i;
				if ((noise_p_data[iArrayIndex] >
				     ph->config_Lmt_FW_Diff_P) ||
				    (noise_p_data[iArrayIndex] <
				     ph->config_Lmt_FW_Diff_N)) {
					noise_p_result = NVT_MP_FAIL;
					noise_p_record[iArrayIndex] = 1;
					pr_info("Noise RawData_Diff_Max Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j,
						 noise_p_data[iArrayIndex],
						 ph->config_Lmt_FW_Diff_N,
						 ph->config_Lmt_FW_Diff_P);
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((noise_n_data[iArrayIndex] >
					...
					noise_n_result = NVT_MP_FAIL;
					noise_n_record[iArrayIndex] = 1;
					pr_info("Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j,
						 noise_n_data[iArrayIndex],
						 ph->config_Lmt_FW_Diff_N,
						 ph->config_Lmt_FW_Diff_P);
...
"Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				iArrayIndex = j * tx_num + i;
				if ((noise_p_data[iArrayIndex] >
				     diff_rawdata_P[iArrayIndex]) ||
				    (noise_p_data[iArrayIndex] <
				     diff_rawdata_N[iArrayIndex])) {
					noise_p_result = NVT_MP_FAIL;
					noise_p_record[iArrayIndex] = 1;
					pr_info("Noise RawData_Diff_Max Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j,
...
"Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
	...
	if (nvt_read_doze_fw_noise(chip_info, ph->config_Doze_Noise_Test_Frame,
				   ph->doze_X_Channel, doze_noise_p_data,
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->doze_X_Channel; i++) {
				...
				if ((doze_noise_n_data[iArrayIndex] >
					...
					pr_info("Doze Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j,
						 doze_noise_n_data[iArrayIndex],
						 ph->config_Lmt_Doze_Diff_N,
						 ph->config_Lmt_Doze_Diff_P);
...
"Doze Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->doze_X_Channel; i++) {
				iArrayIndex = j * ph->doze_X_Channel + i;
				if ((doze_noise_n_data[iArrayIndex] >
				     doze_diff_rawdata_P[iArrayIndex]) ||
				    (doze_noise_n_data[iArrayIndex] <
				     doze_diff_rawdata_N[iArrayIndex])) {
					doze_noise_p_result = NVT_MP_FAIL;
					doze_noise_p_record[iArrayIndex] = 1;
					pr_info("Doze Noise RawData_Diff_Max Test failed at data[%d][%d] = %d [%d,%d]\n",
					    i, j,
...
"Doze Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n",
	...
	if (nvt_read_doze_baseline(chip_info, ph->doze_X_Channel, doze_raw_data,
				   buf_len) != 0) {
		TPD_INFO("read Doze FW Rawdata failed!\n");
		doze_rawdata_result = NVT_MP_FAIL_READ_DATA;
		seq_puts(s, "read Doze FW Rawdata failed!\n");
		err_cnt++;
		goto TEST_END;
	}

	if ((ph->config_Lmt_Doze_Rawdata_P != 0) &&
	    (ph->config_Lmt_Doze_Rawdata_N != 0)) {
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->doze_X_Channel; i++) {
				iArrayIndex = j * ph->doze_X_Channel + i;
				if ((doze_raw_data[iArrayIndex] >
				     ph->config_Lmt_Doze_Rawdata_P) ||
				    (doze_raw_data[iArrayIndex] <
				     ph->config_Lmt_Doze_Rawdata_N)) {
					doze_rawdata_result = NVT_MP_FAIL;
					doze_raw_record[iArrayIndex] = 1;
					pr_info("Doze FW Rawdata Test failed at data[%d][%d] = %d [%d,%d]\n",
					    i, j, doze_raw_data[iArrayIndex],
					    ph->config_Lmt_Doze_Rawdata_N,
					    ph->config_Lmt_Doze_Rawdata_P);
					if (!err_cnt) {
						seq_printf(s,
		"Doze FW Rawdata Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j,
						 doze_raw_data[iArrayIndex],
						 ph->config_Lmt_Doze_Rawdata_N,
						 ph->config_Lmt_Doze_Rawdata_P);
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < ph->doze_X_Channel; i++) {
				iArrayIndex = j * ph->doze_X_Channel + i;
				if ((doze_raw_data[iArrayIndex] >
				     doze_rawdata_P[iArrayIndex]) ||
				    (doze_raw_data[iArrayIndex] <
				     doze_rawdata_N[iArrayIndex])) {
					doze_rawdata_result = NVT_MP_FAIL;
					doze_raw_record[iArrayIndex] = 1;
					pr_info("Doze FW Rawdata Test failed at data[%d][%d] = %d [%d,%d]\n",
					    i, j, doze_raw_data[iArrayIndex],
	...
	if (nvt_read_fw_short(chip_info, short_data, buf_len) != 0) {
		...
		seq_puts(s, "read Short test data failed!\n");
		err_cnt++;
		goto TEST_END;
	}
	if ((ph->config_Lmt_Short_Rawdata_P != 0) &&
	    (ph->config_Lmt_Short_Rawdata_N != 0)) {
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				iArrayIndex = j * tx_num + i;
				if ((short_data[iArrayIndex] >
				     ph->config_Lmt_Short_Rawdata_P) ||
				    (short_data[iArrayIndex] <
				     ph->config_Lmt_Short_Rawdata_N)) {
					short_result = NVT_MP_FAIL;
					short_record[iArrayIndex] = 1;
					pr_info(
					    "Short Test failed at data[%d][%d] = %d [%d,%d]\n",
					    i, j, short_data[iArrayIndex],
					    ph->config_Lmt_Short_Rawdata_N,
					    ph->config_Lmt_Short_Rawdata_P);
					if (!err_cnt) {
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				iArrayIndex = j * tx_num + i;
				if ((short_data[iArrayIndex] >
				     short_rawdata_P[iArrayIndex]) ||
				    (short_data[iArrayIndex] <
				     short_rawdata_N[iArrayIndex])) {
					short_result = NVT_MP_FAIL;
					short_record[iArrayIndex] = 1;
					pr_info("Short Test failed at data[%d][%d] = %d [%d,%d]\n",
						 i, j, short_data[iArrayIndex],
						 short_rawdata_N[iArrayIndex],
						 short_rawdata_P[iArrayIndex]);
					if (!err_cnt) {
	...
	if (nvt_read_fw_open(chip_info, open_data, buf_len) != 0) {
		...
		seq_puts(s, "read Open test data failed!\n");
		err_cnt++;
		goto TEST_END;
	}
	for (j = 0; j < rx_num; j++) {
		for (i = 0; i < tx_num; i++) {
			...
			if ((open_data[iArrayIndex] >
			     open_rawdata_P[iArrayIndex]) ||
			    (open_data[iArrayIndex] <
			     open_rawdata_N[iArrayIndex])) {
				open_result = NVT_MP_FAIL;
				open_record[iArrayIndex] = 1;
				pr_info("Open Test failed at data[%d][%d] = %d [%d,%d]\n",
					 i, j, open_data[iArrayIndex],
					 open_rawdata_N[iArrayIndex],
					 open_rawdata_P[iArrayIndex]);
				if (!err_cnt) {
					seq_printf(s,
			"Open Test failed at data[%d][%d] = %d [%d,%d]\n",
						   i, j, open_data[iArrayIndex],
						   open_rawdata_N[iArrayIndex],
						   open_rawdata_P[iArrayIndex]);
				}
...
TEST_END:
	...
	getnstimeofday(&now_time);
	rtc_time_to_tm(now_time.tv_sec, &rtc_now_time);
	char *prex = "/sdcard/TpTestReport/screenOn";
	...
	/* Project Name */
	store_to_file(nvt_testdata->fd, "CUSTOM_O_%d\n", 19131);
	/* Project ID */
	...
	nvt_store_testrecord_to_file(nvt_testdata->fd, "rawData",
				     rawdata_result, raw_record, tx_num,
				     rx_num);
	/* 2. cc             */
	nvt_store_testrecord_to_file(nvt_testdata->fd, "ccData", cc_result,
				     cc_record, tx_num, rx_num);
	/* 3. noise max      */
	...
	/* Test data */
	/* 1. rawdata        */
	nvt_store_testdata_to_file(nvt_testdata->fd, "rawData", rawdata_result,
				   raw_data, tx_num, rx_num);
	/* 2. cc             */
	nvt_store_testdata_to_file(nvt_testdata->fd, "ccData", cc_result,
				   cc_data, tx_num, rx_num);
	/* 3. noise max      */
...
static int nova_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:NOVA%04X\nFW_VER:0x%02X\nCH:%dX%d\n",
		       0x672C, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
...
static void nova_init_custom_o_apk_op(struct touchpanel_data *ts)
{
...
{
	TPD_INFO("%s  is called\n", __func__);
	tp_i2c_suspend(g_chip_info->ts);

	/* do esd recovery, bootloader reset */
	nvt_reset(g_chip_info);
	tp_touch_btnkey_release();

	tp_i2c_resume(g_chip_info->ts);
	return 0;
}
#endif


/*********** Start of SPI Driver and Implementation of it's
 * callbacks*************************/
int __maybe_unused nvt_tp_probe(struct spi_device *client)
{
	...
	if (ts->esd_handle_support) {
		...
		ts->esd_info.esd_work_time = msecs_to_jiffies(
		    NVT_TOUCH_ESD_CHECK_PERIOD); // change esd check interval to
						 // 1.5s
	...
	if (ts->boot_mode == RECOVERY_BOOT || is_oem_unlocked() ||
		...
		TPD_INFO(
		    "In Recovery mode, no-flash download fw by headfile\n");
		ret = nvt_fw_update(chip_info, NULL, 0);
		if (ret > 0)
			TPD_INFO("fw update failed!\n");
...
#ifdef CONFIG_FB
	.suspend = nvt_spi_suspend, .resume = nvt_spi_resume,
#endif
};
