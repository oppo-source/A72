		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
			"%s: rmi4_data->suspend=%d,rmi4_data->enable_wakeup_gesture=%d,Number of fingers to process = %d,extra_data->data15_size=%d,rmi4_data->report_pressure=%d,finger_data->object_type_and_status=%d\n",
			__func__,rmi4_data->suspend, rmi4_data->enable_wakeup_gesture,fingers_to_process,extra_data->data15_size,rmi4_data->report_pressure,(data + finger)->object_type_and_status);
		...
		bool report)
{
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
				/*	dev_err(rmi4_data->pdev->dev.parent,
				"%s:fhandler->num_of_data_sources=%d,fhandler->intr_mask=%d,intr[fhandler->intr_reg_num]=%d\n",
				__func__,fhandler->num_of_data_sources,fhandler->intr_mask,intr[fhandler->intr_reg_num]);*/
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link) {
				dev_err(rmi4_data->pdev->dev.parent,
				"%s:exp_fhandler->insert=%d,exp_fhandler->remove=%d\n",
				__func__,exp_fhandler->insert,exp_fhandler->remove);
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s:gpio_get_value(bdata->irq_gpio)=%d,bdata->irq_on_state=%d\n",
				__func__,gpio_get_value(bdata->irq_gpio),bdata->irq_on_state);
		...
		bool enable)
{
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		...
		dev_err(rmi4_data->pdev->dev.parent,
					"%s:enable=%d,rmi4_data->num_of_intr_regs=%d,intr_addr=0x%x\n",
					__func__,enable,rmi4_data->num_of_intr_regs,intr_addr);
	}
		...
		bool enable, bool attn_only)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s:1,gpio_get_value(bdata->irq_gpio)=%d,bdata->irq_on_state=%d\n",
				__func__,gpio_get_value(bdata->irq_gpio),bdata->irq_on_state);
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s:2,gpio_get_value(bdata->irq_gpio)=%d,bdata->irq_on_state=%d\n",
				__func__,gpio_get_value(bdata->irq_gpio),bdata->irq_on_state);
	...
	dev_err(rmi4_data->pdev->dev.parent,
					"%s: Clear interrupts.rmi4_data->f01_data_base_addr=0x%x,data[0]=0x%x,rmi4_data->num_of_intr_regs=%d\n",
					__func__,rmi4_data->f01_data_base_addr,data[0],rmi4_data->num_of_intr_regs);
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s:3,gpio_get_value(bdata->irq_gpio)=%d,bdata->irq_on_state=%d\n",
				__func__,gpio_get_value(bdata->irq_gpio),bdata->irq_on_state);
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s:4,gpio_get_value(bdata->irq_gpio)=%d,bdata->irq_on_state=%d\n",
				__func__,gpio_get_value(bdata->irq_gpio),bdata->irq_on_state);
	} else {
		if (rmi4_data->irq_enabled) {
		...
		unsigned int intr_count)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
			"%s: Function %02x rmi4_data->num_of_fingers = %d max x = %d max y = %d\n",
			__func__, fhandler->fn_number,rmi4_data->num_of_fingers,
...
rescan_pdt:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: F%02x found (page %d),rmi_fd.intr_src_count=%d,pdt_entry_addr=0x%02x,rmi_fd.query_base_addr=0x%02x,rmi_fd.ctrl_base_addr=0x%02x,rmi_fd.data_base_addr=0x%02x,rmi_fd.cmd_base_addr=0x%02x\n",
					__func__, rmi_fd.fn_number,
					page_number,rmi_fd.intr_src_count,pdt_entry_addr,rmi_fd.query_base_addr,rmi_fd.ctrl_base_addr,rmi_fd.data_base_addr,rmi_fd.cmd_base_addr);
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->reset_gpio >= 0) {
		...
		dev_err(rmi4_data->pdev->dev.parent,
					"%s:setup gpio_get_value=%d\n",
					__func__,gpio_get_value(bdata->reset_gpio));
		...
		bool insert)
{
	...
	pr_err("%s: exp_fn->fn_type=%d,exp_data.initialized=%d,insert=%d\n",
					__func__,exp_fn->fn_type,exp_data.initialized,insert);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s:0,gpio_get_value(bdata->irq_gpio)=%d,bdata->irq_on_state=%d\n",
				__func__,gpio_get_value(bdata->irq_gpio),bdata->irq_on_state);
