...
#ifdef PLATFORM_DRIVER_NAME
#undef PLATFORM_DRIVER_NAME
...
{
	dev_info(dev, "%s Attempted to read from write-only attribute %s\n",
...
{
	dev_info(dev, "%s Attempted to write to read-only attribute %s\n",
			__func__, attr->attr.name);
	return -EPERM;
}
