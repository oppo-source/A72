			  ...
			  const struct i2c_device_id *id)
{
	...
	int rc = 0;
    pr_err("i2c_pmic i2c_pmic_probe\n");

	chip = devm_kzalloc(&client->dev, sizeof(*chip), GFP_KERNEL);
	if (!chip) {
        pr_err("i2c_pmic cchip error\n");
		return -ENOMEM;
     }
	...
	if (!chip->regmap) {
        pr_err("i2c_pmic cchip->regmap error\n");
		return -ENODEV;
        }
	i2c_set_clientdata(client, chip);
	if (!of_property_read_bool(chip->dev->of_node, "interrupt-controller")) {
        pr_err("i2c_pmic probe_children\n");
		goto probe_children;
	}
	...
	if (!chip->domain) {
		rc = -ENOMEM;
        pr_err("i2c_pmic chip->domain error\n");
		goto cleanup;
	}

	rc = i2c_pmic_parse_dt(chip);
	if (rc < 0) {
		pr_err("i2c_pmic Couldn't parse device tree rc=%d\n", rc);
		goto cleanup;
	}

	rc = i2c_pmic_determine_initial_status(chip);
	if (rc < 0) {
		pr_err("i2c_pmic Couldn't determine initial status rc=%d\n", rc);
		goto cleanup;
	}
	...
	if (chip->pinctrl_name) {
		...
		if (IS_ERR(chip->pinctrl)) {
			pr_err("i2c_pmic Couldn't select %s pinctrl rc=%ld\n",
	...
	if (rc < 0) {
		pr_err("i2c_pmic Couldn't request irq %d rc=%d\n", client->irq, rc);
		goto cleanup;
	}
...
probe_children:
	...
	pr_info("i2c_pmic I2C PMIC probe successful\n");
	return rc;
...
static const struct of_device_id i2c_pmic_match_table[] = {
	{ .compatible = "mediatek,external_pmic", },
	{ },
};
