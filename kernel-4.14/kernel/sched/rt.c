...
#ifdef VENDOR_EDIT
// XieLiujie@BSP.KERNEL.PERFORMANCE, 2020/12/03, Add for animation opt
	if ((sysctl_launcher_boost_enabled == 2) && strstr(task->comm, "surfaceflinger")) {
		int prefer_cpu = 0;
		struct root_domain *rd = cpu_rq(this_cpu)->rd;
		for_each_cpu(prefer_cpu, lowest_mask) {
			if (cpu_isolated(prefer_cpu) && idle_cpu(prefer_cpu) && (prefer_cpu >= rd->max_cap_orig_cpu))
				return prefer_cpu;
		}
	}
#endif
