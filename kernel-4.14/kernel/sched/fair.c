...
static inline bool check_for_ux_migration(struct task_struct *p, int cpu)
{
	return (sysctl_launcher_boost_enabled == 2) && (p->static_ux == 1);
}
...
void check_for_migration(struct rq *rq, struct task_struct *p)
{
	...
	if (rq->misfit_task_load || check_for_ux_migration(p, cpu)) {
